...
void clean_lan_app_samba()
{	
	RunSystemCmd(NULL_FILE, "killall", "-15", "usbStorageAppController", NULL_STR);		
}
...
void start_lan_app_samba()
{
	...
	snprintf(cmdBuffer,sizeof(cmdBuffer),"usbStorageAppController -a %d &",anon_access);
	system(cmdBuffer);
	return;
}


int reinit_func_lan_app_samba(BASE_DATA_Tp data)
{
	if(!data->isStartUp)
		clean_lan_app_samba();
	start_lan_app_samba();
...
int reinit_func_lan_app_minidlna(BASE_DATA_Tp data)
{
	if(!data->isStartUp)
		clean_lan_minidlna();
	start_lan_minidlna();
