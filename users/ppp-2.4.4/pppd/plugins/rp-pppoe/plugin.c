...
//asynchronous run flash command to avoid calling connect() (which will calling pppoe_connect() in pppoe driver) delay and lead to pppoe dial fails
int setSessionIdAndMacToFlash(unsigned short sid, unsigned char *server_mac)
{
	pid_t pid;	
	unsigned char str_sid[16]={0};
	...
	{
		/* the child */	
		snprintf(str_sid, sizeof(str_sid), "%d", sid);
		//execl("/bin/flash", "flash", "set", "PPP_SESSION_NUM", str_sid, NULL);		
		snprintf(cmd_buf,sizeof(cmd_buf),"flash set PPP_SESSION_NUM %s",str_sid);
		system(cmd_buf);

		snprintf(str_mac, sizeof(str_mac), "%02x%02x%02x%02x%02x%02x", server_mac[0],server_mac[1],server_mac[2],server_mac[3],server_mac[4],server_mac[5]);
		//execl("/bin/flash", "flash", "set", "PPP_SERVER_MAC", str_mac, NULL);
		snprintf(cmd_buf,sizeof(cmd_buf),"flash set PPP_SERVER_MAC %s",str_mac);
		system(cmd_buf);	
		exit(0);	
	} 
	else if (pid > 0) 
	{
		/* the parent */
		//avoid child process's state become to Z(zoibem)
...
PPPOEConnectDevice(void)
{
...
#if 0 //Patch for pppoe re-connet to server(winxp) after power off/on
    {
    	char cmdBuf[50];
	...
	setSessionIdAndMacToFlash(sp.sa_addr.pppoe.sid, sp.sa_addr.pppoe.remote);
