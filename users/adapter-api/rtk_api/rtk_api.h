...
struct rtk_link_type_info {
    ...
    unsigned char client_host_ip[16];
#endif
#endif
};
...
typedef enum {WIFI_STATUS_DISABLE, WIFI_STATUS_ENABLE, WIFI_STATUS_NOT_SUPPORT} WIFI_STATUS_T;

struct rtk_dut_info
{
    WIFI_BAND_T band;
    WIFI_STATUS_T enable_2g;
    WIFI_STATUS_T enable_5g;
    int repeater_enable_2g;
    int repeater_enable_5g;
    int wisp_idx;
    int repeater_mode;
    OPMODE_T opmode;
#if defined(CONFIG_ELINK_SUPPORT) || defined(CONFIG_ANDLINK_SUPPORT) || defined(CONFIG_APP_CMCC_ADAPTER)
    char vendor[MAX_INFO_LEN];
    char model[MAX_INFO_LEN];
    char hwversion[MAX_INFO_LEN];
    char swversion[MAX_INFO_LEN];
    char sn[MAX_INFO_LEN];
    char deviceType[MAX_INFO_LEN];
    char productToken[MAX_INFO_LEN];
    char deviceId[MAX_INFO_LEN];
    char deviceToken[MAX_INFO_LEN];
    char andlinkToken[MAX_INFO_LEN];
    char synccode[MAX_INFO_LEN];
    char gwIp[16];
    int HBTime;
    int need_recover;
    int qlink_timeout;
...
typedef struct rtk_wlan_sta_extra_info {
	unsigned short	aid;
	unsigned char	addr[6];
	unsigned long	tx_packets;
	unsigned long	rx_packets;
	...
	char rxrate[10];
    char txrate[10];
} RTK_WLAN_STA_EXTRA_INFO_T, *RTK_WLAN_STA_EXTRA_INFO_Tp;
...
typedef struct meminfo_user
{
	unsigned int mem_free;  	//MB
	unsigned int mem_userTotal;	//MB
}MEMINFO_USER_T, *MEMINFO_USER_Tp;

typedef struct sys_meminfo 
{
	unsigned int ramSizeTotal;   //unit:Mbytes
	unsigned int ramSizeFree;	 //unit:Mbytes
	unsigned int flashSizeTotal; //unit:Mbytes
	unsigned int ramUtility;     //percent(eg:60)
}SYS_MEMINFO_T, *SYS_MEMINFO_Tp;
