...
int rtk_wlan_set_security_wep(unsigned char *ifname, unsigned int auth_type, unsigned int key_len, unsigned int key_type, unsigned int default_key_idx, unsigned char *key)
{
	...
	if(apmib_set_rtkapi(ifname,MIB_WLAN_WEP_DEFAULT_KEY,(void *)&default_key_idx,TYPE_INT) <= 0)
		ret = RTK_FAILED;
	if(key_len == 1){ // wep64
		if( len!=WEP64_KEY_LEN && len!=WEP64_KEY_LEN*2 ){
			printf("Invalid WEP key length %d\n", len);
			return RTK_FAILED;
		}
		if( len==WEP64_KEY_LEN ){
			snprintf(buf, sizeof(buf), "%s", key);
		}else{
			if(!string_to_hex(key, buf, len)){
				printf("Invalid WEP hex\n");
				return RTK_FAILED;
			}
		}
	
		if(default_key_idx == 1){ 
			if(apmib_set_rtkapi(ifname,MIB_WLAN_WEP64_KEY2,(void *)buf,TYPE_ARRAY) <= 0)
				ret = RTK_FAILED;
		}
		else if(default_key_idx == 2){
			if(apmib_set_rtkapi(ifname,MIB_WLAN_WEP64_KEY3,(void *)buf,TYPE_ARRAY) <= 0)
				ret = RTK_FAILED;
		}
		else if(default_key_idx == 3){
			if(apmib_set_rtkapi(ifname,MIB_WLAN_WEP64_KEY4,(void *)buf,TYPE_ARRAY) <= 0)
				ret = RTK_FAILED;
		}
		else{
			if(apmib_set_rtkapi(ifname,MIB_WLAN_WEP64_KEY1,(void *)buf,TYPE_ARRAY) <= 0)
				ret = RTK_FAILED;
		}
	}
	else if(key_len == 2){// wep128
	
		if( len!=WEP128_KEY_LEN && len!=WEP128_KEY_LEN*2 ){
			printf("Invalid WEP key length %d\n", len);
			return RTK_FAILED;
		}
		if( len==WEP128_KEY_LEN ){
			snprintf(buf, sizeof(buf), "%s", key);
		}else{
			if(!string_to_hex(key, buf, len)){
				printf("Invalid WEP hex\n");
		...
		if(default_key_idx == 1){
			if(apmib_set_rtkapi(ifname,MIB_WLAN_WEP128_KEY2,(void *)buf,TYPE_ARRAY) <= 0)
				ret = RTK_FAILED;
		}
		else if(default_key_idx == 2){
			if(apmib_set_rtkapi(ifname,MIB_WLAN_WEP128_KEY3,(void *)buf,TYPE_ARRAY) <= 0)
				ret = RTK_FAILED;
		}
		else if(default_key_idx == 3){
			if(apmib_set_rtkapi(ifname,MIB_WLAN_WEP128_KEY4,(void *)buf,TYPE_ARRAY) <= 0)
				ret = RTK_FAILED;
		}
		else{
			if(apmib_set_rtkapi(ifname,MIB_WLAN_WEP128_KEY1,(void *)buf,TYPE_ARRAY) <= 0)
				ret = RTK_FAILED;
		}
	}
#elif defined(HAVE_RTK_DRVMIB)
...
int rtk_wlan_set_security_wpa(unsigned char *ifname, unsigned int auth_type, unsigned int cipher_suite, unsigned int key_type, unsigned char *key)
{
	...
	int len = strlen(key);	
	if( key_type==KEY_ASCII )
		if(len<8 || len>64){
			printf("Invalid wpa key length %d\n", len);
			return RTK_FAILED;
		}
...
/**************************************************
* @NAME:
* 	rtk_wlan_get_real_channel
* 
* @PARAMETERS:
* 	@Input
* 		ifname: Wireless interface name
* 	@Output
*	 	channel: the channel number of wlan in driver
*
* @RETRUN:
* 	RTK_SUCCESS
*	RTK_FAILED
* 
* @FUNCTION :
* 	Get the channel of wlan from driver
*
***************************************************/

int rtk_wlan_get_real_channel(unsigned char *ifname, unsigned int *channel)
{
    int ret = RTK_SUCCESS;
    if(!ifname || !channel)
        return RTK_FAILED;
    ...
    if(apmib_get_rtkapi(ifname,MIB_WLAN_CHANNEL,(void *)channel,TYPE_INT) <= 0)
        ret = RTK_FAILED;
    if(*channel == 0){
        char cmd[64], line_buff[64];
        char* pchar;
        FILE *fp;
        sprintf(cmd, "iwpriv %s get_mib channel", ifname);
        if((fp = popen(cmd, "r")) != NULL){
            fgets(line_buff, sizeof(line_buff), fp);
            pchar = strstr(line_buff, "get_mib:");
            pchar += strlen("get_mib:");
            *channel = atoi(pchar);
            pclose(fp);
        }else{
            return RTK_FAILED;
...
int rtk_wlan_get_rf_band(unsigned char *ifname, unsigned int* rfband)
{
	int ret = RTK_SUCCESS;
	if(!ifname || !rfband)
		return RTK_FAILED;

	if(apmib_get_rtkapi(ifname, MIB_WLAN_PHY_BAND_SELECT,(void *)rfband, TYPE_INT) <= 0)
		ret = RTK_FAILED;

	return ret; 
}

int rtk_wlan_set_rf_band(unsigned char *ifname, unsigned int rf_band)
{
	int intValue, ret = RTK_SUCCESS;
	if(!ifname)
		return RTK_FAILED;
	...
	intValue = rf_band;
	if(apmib_set_rtkapi(ifname,MIB_WLAN_PHY_BAND_SELECT,(void *)&intValue,TYPE_INT) <= 0)
		ret = RTK_FAILED;

	return ret; 
}

int rtk_wlan_get_2g5g_support(unsigned int *support)
{
	int ret = RTK_SUCCESS;
	unsigned int wlanBand2G5GSelect, rf_band;
	if(!support)
		return RTK_FAILED;
...
int hex2String(unsigned char *hex,unsigned char *dest, int len)  
{
    char buf[256];  
	unsigned char hb, lb;  
	int cnt=0, i;
...
int rtk_wlan_get_enc_code(unsigned char *ifname, unsigned int enc, unsigned char *code, unsigned int code_len)
{
	int ret = RTK_SUCCESS;
	unsigned char wlan_wpa_psk[100];
	unsigned char wlan_wep64_key1[100], wlan_wep64_key2[100], wlan_wep64_key3[100], wlan_wep64_key4[100];
	unsigned char wlan_wep128_key1[100], wlan_wep128_key2[100], wlan_wep128_key3[100], wlan_wep128_key4[100];
	unsigned int wlan_wep;
	char tmpbuf[100], tmp1[100], tmp2[100];
	unsigned int keyfmt = KEY_ASCII; 	//0:passphrase 	1:hex
	int i;
	int key_len;
	...
	if(enc==ENCRYPT_DISABLED){
		snprintf(code, code_len, "%s", "");
	}else if(enc==ENCRYPT_WEP){
		if(apmib_get_rtkapi(ifname, MIB_WLAN_WEP, (void*)&wlan_wep ,TYPE_INT) <= 0)
			ret = RTK_FAILED;
		if (wlan_wep <= WEP_DISABLED || wlan_wep > WEP128) {
			printf("[%s %d]WEP encrypt length error\n", __FUNCTION__,__LINE__);			
			return RTK_FAILED;
		}
		if(apmib_get_rtkapi(ifname, MIB_WLAN_WEP_KEY_TYPE, (void*)&keyfmt ,TYPE_INT) <= 0){
			printf("[%s %d]WEP keyfmt error\n", __FUNCTION__,__LINE__);			
			return RTK_FAILED;
		}

		if(wlan_wep==WEP64){
			if(apmib_get_rtkapi(ifname, MIB_WLAN_WEP64_KEY1, (void*)&wlan_wep64_key1 ,TYPE_ARRAY) <= 0)
				ret = RTK_FAILED;
			if(apmib_get_rtkapi(ifname, MIB_WLAN_WEP64_KEY2, (void*)&wlan_wep64_key2 ,TYPE_ARRAY) <= 0)
				ret = RTK_FAILED;
			if(apmib_get_rtkapi(ifname, MIB_WLAN_WEP64_KEY3, (void*)&wlan_wep64_key3 ,TYPE_ARRAY) <= 0)
				ret = RTK_FAILED;
			if(apmib_get_rtkapi(ifname, MIB_WLAN_WEP64_KEY4, (void*)&wlan_wep64_key4 ,TYPE_ARRAY) <= 0)
				ret = RTK_FAILED;				
			
			if(keyfmt==KEY_HEX){							
				convert_bin_to_str((unsigned char *)wlan_wep64_key1, WEP64_KEY_LEN, tmp1);				
			}else{				
				snprintf(tmp1, sizeof(tmp1), "%s", wlan_wep64_key1);				
			}			
			
			/* just wep_key_idx = 1 is used in wlan driver */			
			snprintf(code, code_len, "%s", tmp1);			
		}else {
			if(apmib_get_rtkapi(ifname, MIB_WLAN_WEP128_KEY1, (void*)&wlan_wep128_key1 ,TYPE_ARRAY) <= 0)
				ret = RTK_FAILED;
			if(apmib_get_rtkapi(ifname, MIB_WLAN_WEP128_KEY2, (void*)&wlan_wep128_key2 ,TYPE_ARRAY) <= 0)
				ret = RTK_FAILED;
			if(apmib_get_rtkapi(ifname, MIB_WLAN_WEP128_KEY3, (void*)&wlan_wep128_key3 ,TYPE_ARRAY) <= 0)
				ret = RTK_FAILED;
			if(apmib_get_rtkapi(ifname, MIB_WLAN_WEP128_KEY4, (void*)&wlan_wep128_key4 ,TYPE_ARRAY) <= 0)
				ret = RTK_FAILED;				
			
			if(keyfmt==KEY_HEX){
				convert_bin_to_str((unsigned char *)wlan_wep128_key1, WEP128_KEY_LEN, tmp1);							
			}else {				
				snprintf(tmp1, sizeof(tmp1), "%s", wlan_wep128_key1);								
			}		
		...
		if(apmib_get_rtkapi(ifname, MIB_WLAN_WPA_PSK, (void*)&wlan_wpa_psk ,TYPE_ARRAY) <= 0)
			ret = RTK_FAILED;
		snprintf(code, code_len, "%s", wlan_wpa_psk);
	}else if(enc==ENCRYPT_WAPI){
			// TODU
	}else {
		printf("[%s %d]Invalid Enc type %d\n", __FUNCTION__,__LINE__,enc);		
		return RTK_FAILED;
	}		
#endif
	return ret; 
...
int rtk_wlan_get_cipher_suite(unsigned char *ifname, unsigned int *auth_type, unsigned int *cipher_suite, unsigned int *cipher2_suite)
{
	int ret = RTK_SUCCESS;
#if defined(HAVE_RTK_APMIB) && !defined(HAVE_RTK_DRVMIB)	
	if(apmib_get_rtkapi(ifname,MIB_WLAN_WPA_CIPHER_SUITE,(void *)cipher_suite,TYPE_INT) <= 0)
		ret = RTK_FAILED;
	if(apmib_get_rtkapi(ifname,MIB_WLAN_WPA2_CIPHER_SUITE,(void *)cipher2_suite,TYPE_INT) <= 0)
		ret = RTK_FAILED;
	if(apmib_get_rtkapi(ifname,MIB_WLAN_WPA_AUTH,(void *)auth_type,TYPE_INT) <= 0)
		ret = RTK_FAILED;
#endif
...
int rtk_wlan_get_wpa_auth(unsigned char *ifname, unsigned int *auth_type)
{
	int ret = RTK_SUCCESS;
#if defined(HAVE_RTK_APMIB) && !defined(HAVE_RTK_DRVMIB)	
	if(apmib_get_rtkapi(ifname,MIB_WLAN_WPA_AUTH,(void *)auth_type,TYPE_INT) <= 0)
...
int rtk_wlan_set_wpa_auth(unsigned char *ifname, unsigned int auth_type)
{
	int ret = RTK_SUCCESS;
	unsigned int value = auth_type;
	
	if(apmib_set_rtkapi(ifname,MIB_WLAN_WPA_AUTH,(void *)&value,TYPE_INT) <= 0)
		ret = RTK_FAILED;
	return ret; 
}

int rtk_wlan_get_wep_auth(unsigned char *ifname, unsigned int *auth_type)
{
	int ret = RTK_SUCCESS;
#if defined(HAVE_RTK_APMIB) && !defined(HAVE_RTK_DRVMIB)	
	if(apmib_get_rtkapi(ifname,MIB_WLAN_AUTH_TYPE,(void *)auth_type,TYPE_INT) <= 0)
...
int rtk_wlan_set_wep_auth(unsigned char *ifname, unsigned int auth_type)
{
	int ret = RTK_SUCCESS;
	unsigned int value = auth_type;
	
	if(apmib_set_rtkapi(ifname,MIB_WLAN_AUTH_TYPE,(void *)&value,TYPE_INT) <= 0)
		ret = RTK_FAILED;
	return ret; 
}

int rtk_wlan_get_key_type_by_encrypt(unsigned char *ifname, unsigned int encrypt, unsigned char *key, unsigned int *wlan_wep, unsigned int *keyfmt)
{
	int ret = RTK_SUCCESS;
    ...
    if( encrypt== ENCRYPT_WEP ){
        if( key_len==WEP64_KEY_LEN ){
            *keyfmt = KEY_ASCII;
            *wlan_wep = WEP64;
        }else if( key_len==WEP64_KEY_LEN*2 ){
             *keyfmt = KEY_HEX;
             *wlan_wep = WEP64;
        }else if( key_len==WEP128_KEY_LEN){
            *keyfmt = KEY_ASCII;
            *wlan_wep = WEP128;
        }else if( key_len==WEP128_KEY_LEN*2 ){
            *keyfmt = KEY_HEX;
            *wlan_wep = WEP128;
        }else {
            printf("[%s %d]Invalid wep key length %d !\n", __FUNCTION__, __LINE__,key_len);
	        return RTK_FAILED;
        } 
...
int rtk_wlan_set_wlan_enable(unsigned char *ifname, unsigned int enable)
{
	int ret = RTK_SUCCESS;
...
int rtk_wlan_set_wlan_accelerate(unsigned char *ifname, unsigned int accelerate)
{
	int ret = RTK_SUCCESS;
	unsigned int intValue = accelerate;
	
	if(apmib_set_rtkapi(ifname,MIB_WLAN_QOS_ENHANCE,(void *)&intValue,TYPE_INT) <= 0)
		ret = RTK_FAILED;
	return ret; 
}
...
int getWispRptIfaceName(char*pIface,int wlanId)
{
	...
	rtk_wlan_get_mode(wlan_wanIfName, (void *)&wlanMode);	
...
int rtk_wlan_down_wlan(unsigned char *wlan_interface)
{	
	...
	unsigned int wlan_enable = 0, mc2u_disabled=0;
	int i = 0;
	...
	wlan_index =  get_wlan_idx_by_wlanif(wlan_interface);
	...
	if(wlan_virtual_interface[0])
	{
		...
		snprintf(tmpBuff, sizeof(tmpBuff), "%s", wlan_virtual_interface);
	...
	for(i=0;i<MAX_WDS_NUM;i++) {
		snprintf(tmpBuff, sizeof(tmpBuff), "%s-wds%d", wlan_interface, i);
	...
	if (wlan_support != 0)
	{
		rtk_wlan_get_enable(wlan_interface, (unsigned int *)&wlan_enable);			  
		if(wlan_enable == 0)
		{
		...
		else
		{
			...
			rtk_wlan_get_mc2u(wlan_interface, &mc2u_disabled);
			if(mc2u_disabled)
			{
...
int rtk_wlan_up_wlan(unsigned char *wlan_interface)
{		
	char wlan_virtual_interface[80] = {0};
	char tmpBuff[512] = {0};
	char tmpBuff2[32] = {0};
	...
	int wlan_mode_root=0, wlan_root_disabled=0,wlan_wds_enabled=0,wlan_wds_num=0,wlan_mode=0,i=0;
	int enable = 0, root_enable=0;
	...
	rtk_wlan_get_enable(wlan_interface, (unsigned int *)&root_enable);		
	if(root_enable)
	{
	...
	/*start wds interfaces*/
	rtk_wlan_get_wds(wlan_interface, (void *)&wlan_wds_enabled);
	rtk_wlan_get_wds_entry_num(wlan_interface, (void *)&wlan_wds_num);
	rtk_wlan_get_mode(wlan_interface, (void *)&wlan_mode);	
	if(root_enable && wlan_wds_enabled !=0 && wlan_wds_num !=0 && (wlan_mode==WDS_MODE || wlan_mode==AP_WDS_MODE))
	{
		for(i=0;i<wlan_wds_num;i++){
			sprintf(tmpBuff, "%s-wds%d", wlan_interface, i);
	...
	/*start vxd interface*/	
	wlan_idx = get_wlan_idx_by_wlanif(wlan_interface);
	if(wlan_idx == 0)
	{
	...
	if(repeater_enabled == 1 && root_enable)
	{
		if(opmode == WISP_MODE)
	...
	if(wlan_virtual_interface[0])
	{
		...
		do{
			...
			{
				rtk_wlan_get_enable(token, (unsigned int *)&enable);			
				if(enable && root_enable)
				{
					set_wlan_mib(token);
...
/**************************************************
* @NAME:
* 	rtk_wlan_get_wlan_sta_num
* 
* @PARAMETERS:
* 	@Input
* 		ifname: Wireless interface name
* 	@Output
*		num: the connected sta num
* @RETRUN:
* 	RTK_SUCCESS
*	RTK_FAILED
* 
* @FUNCTION :
* 	Get the sta num of wlan
*
***************************************************/
int rtk_wlan_get_wlan_sta_num( char *interface, int *num )
{
    int skfd=0;
...
/**************************************************
* @NAME:
* 	rtk_wlan_get_wlan_sta_extra_info
* 
* @PARAMETERS:
* 	@Input
* 		ifname: Wireless interface name
* 	@Output
*		pInfo: the storage of infomation
*		the size should be sizeof(RTK_WLAN_EXTRA_STA_INFO_Tp) * (MAX_STA_NUM+1)
* @RETRUN:
* 	RTK_SUCCESS
*	RTK_FAILED
* 
* @FUNCTION :
* 	Get the stations infomation of wlan
*     Extra info: hostname/hostip
*
***************************************************/
int rtk_wlan_get_wlan_sta_extra_info( unsigned char *ifname,  RTK_WLAN_STA_EXTRA_INFO_Tp pInfo)
{
	if(!ifname || !pInfo)
		return RTK_FAILED;
    ...
    wrq.u.data.pointer = (caddr_t)pInfo;
    wrq.u.data.length = sizeof(RTK_WLAN_STA_EXTRA_INFO_T) * (MAX_STA_NUM+1);
    memset(pInfo, 0, sizeof(RTK_WLAN_STA_EXTRA_INFO_T) * (MAX_STA_NUM+1));

    if (iw_get_ext(skfd, ifname, SIOCGIWRTLSTAEXTRAINFO, &wrq) < 0){
    	close( skfd );
		return RTK_FAILED;
	}
    close( skfd );	
