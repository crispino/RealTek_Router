...
int rtk_set_wan_dhcp(struct rtk_dhcp_config *pdhcp_config)
{
	...
	{
		if(isValidName(pdhcp_config->host_name)==RTK_FAILED)
		{
...
int rtk_start_pppoe(void)
{
	...
	char filename[50]={0}, strPID[10]={0};
	int wan_dhcp, pid=-1;
	...
	//kill existed udhcpc on wan interface
	apmib_get(MIB_WAN_DHCP, (void *)&wan_dhcp);
	if(wan_dhcp==3){
		snprintf(filename, sizeof(filename), "%s", "/var/udhcpc/udhcpc-eth1.pid");
