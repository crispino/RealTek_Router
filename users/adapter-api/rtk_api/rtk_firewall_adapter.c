...
{	
	...
	unsigned char buffer[128],__ip_str[32],mac_str[32];	
	unsigned char macEntry[30];
#if 0   //seems wrong here! mac should not be changed
	for(index = 0 ; index < 18; ++index)
	{
		if((mac[index]  >= 'a')  && (mac[index]<='f'))
	  		mac[index] -= 32;
...
//added by elaine, for andlink if5
int rtk_set_qos_req(QOS_REQ_T *req)
{
	unsigned char str_mac[32] = {0}, str_tmp[32] = {0}, str_mac_hex[MAC_ADDR_LEN] = {0};
	char *ptr = NULL, *strptr = NULL;
	int count = 0, index = 0;
	int qos_switch = 0, up_rate, down_rate, QoS_Enabled;
	double up_rate_tmp, down_rate_tmp;
	IPQOS_T entry;
	...
	if(QoS_Enabled == 0){
		QoS_Enabled = 1;
		printf("(%s)%d change QoS enabled to %d\n", __FUNCTION__, __LINE__,QoS_Enabled);
		apmib_set( MIB_QOS_ENABLED, (void *)&QoS_Enabled);
		req->mib_changed = 1;
	}
...
//	if(!string_to_hex(str_mac, str_mac_hex, 12)){
	if(!string_to_hex(req->clientMAC, str_mac_hex, 12)){
	...
	// rate:Mbps --> Kbps
	up_rate_tmp = atof(req->maxUplinkRate);
	if(up_rate_tmp == NO_RATE_LIMIT)
		up_rate = MAX_LINK_RATE;
	else
		up_rate = up_rate_tmp *1024;
	
	down_rate_tmp = atof(req->maxDownlinkRate);
	if(down_rate_tmp == NO_RATE_LIMIT)
		down_rate = MAX_LINK_RATE;
	else
		down_rate = down_rate_tmp * 1024;

	//printf("[====%s %d====] up_rate is %d, down_rate is %d\n",__FUNCTION__,__LINE__, up_rate, down_rate);
	//printf("[====%s %d====] MAX_LINK_RATE is %d\n",__FUNCTION__,__LINE__,MAX_LINK_RATE);
	...
	for(index = 1; index <= count; index++){
		...
		*((char *)&entry) = (char)index;
		...
		if(memcmp(entry.mac, str_mac_hex, MAC_ADDR_LEN) == 0){
			if(entry.enabled==1 && up_rate==entry.bandwidth && down_rate==entry.bandwidth_downlink){
				printf("(%s)%d %s already store in QOS TBL\n",__FUNCTION__,__LINE__,req->clientMAC);
				return RTK_SUCCESS;		// exactly the same
			}
			else{
				printf("(%s)%d Update %s in QUS TBL\n",__FUNCTION__,__LINE__,req->clientMAC);
				if((up_rate == MAX_LINK_RATE) && (down_rate == MAX_LINK_RATE)){
					printf("(%s)%d %s no rate limit, delete Qos Entry\n",__FUNCTION__,__LINE__,req->clientMAC);
					if(!apmib_set(MIB_QOS_DEL, (void *)&entry)){
			        	printf("[%s:%d] Delete Qos Entry Error!\n", __FUNCTION__, __LINE__);
						return RTK_FAILED;
		        	}
				}else{
					IPQOS_T qosArray[2];
					...
					entry.enabled = 1; //entry.enabled = qos_switch;
	...
	// Add a new entry
	printf("(%s)%d Add new entry to QOS TBL\n",__FUNCTION__,__LINE__);
	memset(&entry, '\0', sizeof(IPQOS_T));
	entry.bandwidth 			= up_rate;
	entry.bandwidth_downlink	= down_rate;
	entry.enabled 				= 1; //qos_switch;
...
//added by elaine, for andlink if5
int rtk_set_client_access_control_req(CLIENT_ACCESS_CONTROL_REQ_T *req)
{
	MACFILTER_T entry;
	char str_mac[32] = {0};
	int entryNum = 0, i = 0, mode, macfilter_enabled;
	
	if(req == NULL)
		return RTK_FAILED;

	apmib_get(MIB_MACFILTER_ENABLED, (void *)&macfilter_enabled);
	if(macfilter_enabled == 0){
		macfilter_enabled = 1;
		printf("(%s)%d change MACFILTER enabled to %d\n", __FUNCTION__, __LINE__,macfilter_enabled);
		apmib_set(MIB_MACFILTER_ENABLED, (void *)&macfilter_enabled);
		req->mib_changed = 1;
	}
	...
	if(mode != req->policy){
		mode = req->policy;
		printf("(%s)%d change MACFILTER mode to %d\n", __FUNCTION__, __LINE__,mode);
		apmib_set(MIB_MACFILTER_MODE, (void *)&mode);
		req->mib_changed = 1;
	}
	...
	for(i = 1; i <= entryNum; i++) {
		...
		*((char *)&entry) = (char)i;
		...
		if(strncasecmp(req->clientMAC, str_mac, sizeof(req->clientMAC)) == 0){
			if(req->policy != entry.mode){ //change entry mode
				printf("(%s) Change entry mode for MAC:%s\n",__FUNCTION__,__LINE__,req->clientMAC);
				apmib_set(MIB_MACFILTER_DEL, (void *)&entry);

				entry.mode = req->policy;
...
#endif			
			printf("(%s)%d Add new entry to MACFILTER TBL\n",__FUNCTION__,__LINE__);
			memset(&entry, '\0', sizeof(entry));
			if(!string_to_hex(req->clientMAC, entry.macAddr, 12)){
				printf("[%s:%d] Invalid Mac Address!\n", __FUNCTION__, __LINE__);
				return RTK_FAILED;
			}

			entry.mode = req->policy;
			snprintf((char *)entry.comment, COMMENT_LEN, req->name);
