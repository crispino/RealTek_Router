...
#ifdef CONFIG_APP_CMCC_ADAPTER
// ========= upgrade image =======
RTK_DECLARE_TEST_FUNC(get_DEVUPDATE_INFO_REQ)
{
	struct devUpdate_info_req req={0};
	CU_ASSERT_EQUAL(cmcc_get_DEVUPDATE_INFO_REQ(&req),RTK_SUCCESS);
	printf("\ndevID=%s\n",req.devID);
	printf("type=%s\n",req.type);
	printf("currentVersion=%s\n",req.currentVersion);
}
RTK_DECLARE_TEST_FUNC(set_DEVUPDATE_INFO_RSP)
{
	struct devUpdate_info_rsp rsp={0};
	strcpy(rsp.needUpdate,"1");
	strcpy(rsp.updateDescription,"test");
	strcpy(rsp.updateUrl,"http://192.168.8.209/fw.bin");
	CU_ASSERT_EQUAL(cmcc_set_DEVUPDATE_INFO_RSP(&rsp),RTK_SUCCESS);
	
}
RTK_DECLARE_TEST_FUNC(set_DEVUPDATE_REQ)
{
	struct devUpdate_req req={0};
	strcpy(req.updateUrl,"http://192.168.8.209/fw.bin");
	CU_ASSERT_EQUAL(cmcc_set_DEVUPDATE_REQ(&req),RTK_SUCCESS);
}
RTK_DECLARE_TEST_FUNC(get_DEVUPDATE_RSP)
{
	CU_ASSERT_EQUAL(cmcc_get_DEVUPDATE_RSP(),RTK_SUCCESS);
}
RTK_DECLARE_TEST_FUNC(get_DEVUPDATE_STATUS)
{
	int status=-1;
	CU_ASSERT_EQUAL(cmcc_get_DEVUPDATE_STATUS(&status),RTK_SUCCESS);
	printf("\nstatus=%d\n",status);
}
RTK_DECLARE_TEST_FUNC(set_DEVUPDATE_STATUS)
{
	int status = 1;
	CU_ASSERT_EQUAL(cmcc_set_DEVUPDATE_STATUS(status),RTK_SUCCESS);
	CU_ASSERT_EQUAL(cmcc_get_DEVUPDATE_STATUS(&status),RTK_SUCCESS);
	printf("\nstatus=%d\n",status);
	status =0;
	CU_ASSERT_EQUAL(cmcc_set_DEVUPDATE_STATUS(status),RTK_SUCCESS);
...
//==== get network info =====
RTK_DECLARE_TEST_FUNC(get_BROADBAND_SETTING_REQ)
{
	CU_ASSERT_EQUAL(cmcc_get_BROADBAND_SETTING_REQ(),RTK_SUCCESS);
}
RTK_DECLARE_TEST_FUNC(get_BROADBAND_SETTING_RSP)
{
	struct boardBand_setting rsp={0};
	CU_ASSERT_EQUAL(cmcc_get_BROADBAND_SETTING_RSP(&rsp),RTK_SUCCESS);
	printf("\ntype=%s",rsp.type);
	printf("\nPPPoEName=%s",rsp.PPPoEName);
	printf("\nPPPoECode=%s",rsp.PPPoECode);
	printf("\nIP=%s",rsp.IP);
	printf("\ngatewayIP=%s",rsp.gatewayIP);
	printf("\nsubmask=%s",rsp.submask);
	printf("\nDNS1=%s",rsp.DNS1);
	printf("\nDNS2=%s\n",rsp.DNS2);
}
RTK_DECLARE_TEST_FUNC(set_BROADBAND_SETTING_REQ)
{
	struct boardBand_setting req={0};
	snprintf(req.type,sizeof(req.type),"4");
	snprintf(req.IP,sizeof(req.IP),"192.168.3.234");
	snprintf(req.gatewayIP,sizeof(req.gatewayIP),"192.168.3.200");
	snprintf(req.submask,sizeof(req.submask),"255.255.255.0");
	snprintf(req.DNS1,sizeof(req.DNS1),"192.168.3.180");
	snprintf(req.DNS2,sizeof(req.DNS2),"192.168.3.200");
	CU_ASSERT_EQUAL(cmcc_set_BROADBAND_SETTING_REQ(&req),RTK_SUCCESS);
	CU_ASSERT_EQUAL(cmcc_get_BROADBAND_SETTING_RSP(&req),RTK_SUCCESS);
	printf("\ntype=%s",req.type);
	printf("\nPPPoEName=%s",req.PPPoEName);
	printf("\nPPPoECode=%s",req.PPPoECode);
	printf("\nIP=%s",req.IP);
	printf("\ngatewayIP=%s",req.gatewayIP);
	printf("\nsubmask=%s",req.submask);
	printf("\nDNS1=%s",req.DNS1);
	printf("\nDNS2=%s\n",req.DNS2);
	snprintf(req.type,sizeof(req.type),"1");
	snprintf(req.PPPoEName,sizeof(req.PPPoEName),"chap180");
	snprintf(req.PPPoECode,sizeof(req.PPPoECode),"chap180");
	CU_ASSERT_EQUAL(cmcc_set_BROADBAND_SETTING_REQ(&req),RTK_SUCCESS);
	CU_ASSERT_EQUAL(cmcc_get_BROADBAND_SETTING_RSP(&req),RTK_SUCCESS);
	printf("\ntype=%s",req.type);
	printf("\nPPPoEName=%s",req.PPPoEName);
...
RTK_DECLARE_TEST_FUNC(set_BROADBAND_SETTING_RSP)
{
	CU_ASSERT_EQUAL(cmcc_set_BROADBAND_SETTING_RSP(),RTK_SUCCESS);
}
...
#ifdef CONFIG_APP_CMCC_ADAPTER
CU_TestInfo cmcc_testcases[] ={
//======= upgrade image ===========
	RTK_TEST_INFO_DEF(get_DEVUPDATE_INFO_REQ),
	RTK_TEST_INFO_DEF(set_DEVUPDATE_INFO_RSP),
	RTK_TEST_INFO_DEF(set_DEVUPDATE_REQ),
	RTK_TEST_INFO_DEF(get_DEVUPDATE_RSP),
	RTK_TEST_INFO_DEF(get_DEVUPDATE_STATUS),
	RTK_TEST_INFO_DEF(set_DEVUPDATE_STATUS),	
//========== get network info ====
	RTK_TEST_INFO_DEF(get_BROADBAND_SETTING_REQ),
	RTK_TEST_INFO_DEF(get_BROADBAND_SETTING_RSP),
	RTK_TEST_INFO_DEF(set_BROADBAND_SETTING_REQ),
	RTK_TEST_INFO_DEF(set_BROADBAND_SETTING_RSP),
...
CU_SuiteInfo suites[] = {
#ifdef CONFIG_APP_CMCC_ADAPTER
	{"Testing the cmcc api function:",InitSuite, EndSuite, cmcc_testcases},
#endif
