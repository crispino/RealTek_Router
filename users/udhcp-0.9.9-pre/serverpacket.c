...
int sendACK(struct dhcpMessage *oldpacket, u_int32_t yiaddr)
{
...

	{
		...
		tmp_in_addr.s_addr = packet.yiaddr;
		syslog(LOG_INFO, "dhcpd provide ip %s to device %02x:%02x:%02x:%02x:%02x:%02x", 
			inet_ntoa(tmp_in_addr), packet.chaddr[0], packet.chaddr[1], packet.chaddr[2], packet.chaddr[3], packet.chaddr[4], packet.chaddr[5]);
	}
	return 0;
}
