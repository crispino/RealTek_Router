...
// --------------- USB3G wanType ---------------
   ...
   if (form.wan_macAddr != null){
	...
	if (!checkFieldDigitRange(form.INPUT_VlanId, 1, 4094, vlan_8021q_vid))
		return false;
...
/*==============================================================================*/
/*   wlbasic.htm */
function enableWLAN(form, wlan_id, isCMCCSupport)
{
	var idx_value= form.elements["band"+wlan_id].selectedIndex;
	var band_value= form.elements["band"+wlan_id].options[idx_value].value;
  ...
  if (mode_value !=1) {	//mode != client
	...
	if(isCMCCSupport==1)
		enableTextField(form.elements["ssidpri"+wlan_id]);
  }
  else {	// mode == client
	...
	if(isCMCCSupport==1)
		disableTextField(form.elements["ssidpri"+wlan_id]);
  }
  if (disableSSID[wlan_id]){
...
function disableWLAN(form, wlan_id, isCMCCSupport)
{
  disableTextField(form.elements["mode"+wlan_id]);
...
function updateIputState(form, wlan_id, isCMCCSupport)
{
  if (form.elements["wlanDisabled"+wlan_id].checked)
 	disableWLAN(form, wlan_id, isCMCCSupport);
  else
  	enableWLAN(form, wlan_id, isCMCCSupport);
}
...
function updateMode(form, wlan_id, isCMCCSupport)
{
	var chan_boundid;
	...
	if(bandIdx[wlan_id] == 9 || bandIdx[wlan_id] == 10 ||  bandIdx[wlan_id] == 7 || bandIdx[wlan_id] == 11 || bandIdx[wlan_id] == 14 || bandIdx[wlan_id] == 63 || bandIdx[wlan_id] == 71 || bandIdx[wlan_id] == 75){
	  ...
	  updateIputState(form, wlan_id, isCMCCSupport);  
...
function updateType(form, wlan_id, isccmc)
{
	var mode_selected=0;
	var Type_selected=0;
	var index_channelbound=0;
  updateChan(form, wlan_id);
  updateIputState(form, wlan_id, isccmc); 
...
function showchannelbound_updated(form, band, wlan_id, rf_num)
{
 ...
 if(band == 10 || band == 9|| band == 7)
 form.elements["channelbound"+wlan_id].options[idx++] = new Option("20/40MHz", "3", false, false);
