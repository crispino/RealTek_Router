...
////////////////////////////////////////////////////////////////////////////////
#ifdef DEFAULT_SETTING_CHECK
void default_check(int id)
{
	int special_mib_id[30]={MIB_DHCP_LEASE_TIME,MIB_PPTP_SERVER_IP_ADDR,MIB_L2TP_SERVER_IP_ADDR,\
							MIB_PPP_SESSION_NUM,MIB_PPP_SERVER_MAC,MIB_HW_WSC_PIN\
						};
	int array_index=8;//array_index equal to reality element number of special_mib_id
	//printf("default_check id=%d\n",id);
	//printf("default_check isdefaultsetting=%d\n",pMib->isdefaultsetting);

	struct stat status;
	if(pMib == NULL)
			return;
	else if(pMib->isdefaultsetting == 0 )
		return;
	
#ifdef CONFIG_ANDLINK_SUPPORT
	if ( stat(ANDLINK_SET_MIB_DIR, &status) >= 0)
		return;
#endif	

	
	int is_device_power_on;
	if ( stat(REINIT_FILE, &status) < 0)
		is_device_power_on=1;
	else
		is_device_power_on=0;
	//printf("default_check isFileExist(REINIT_FILE)=%d\n",is_device_power_on);
	if(is_device_power_on)
		return;
	
#ifdef TR181_V6_SUPPORT
	special_mib_id[array_index++]=MIB_DNS_CLIENT_SERVER_MOD;
#endif
#ifdef CONFIG_8021Q_VLAN_SUPPORTED
	special_mib_id[array_index++]=MIB_VLAN_TBL_NUM;
	special_mib_id[array_index++]=MIB_VLAN_OPMODE;
	special_mib_id[array_index++]=MIB_VLAN_PVID_ARRAY;
	special_mib_id[array_index++]=MIB_VLAN_HW_NAT_LAN_VID;
	special_mib_id[array_index++]=MIB_VLAN_DELALL;
#endif
#ifdef CONFIG_APP_BOA_HE_UI
	special_mib_id[array_index++]=MIB_SITE_ACCESS;
#endif

	if(pMib->isdefaultsetting == 1)
	{
		int index,is_special_mib=0;
		for(index=0;index<array_index;index++)
		{
			if(id==special_mib_id[index])
				{
					is_special_mib=1;
					break;
				}
