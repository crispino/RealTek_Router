...
static void setTaskTimer(char *file)
{
	FILE *fp;
	char line_buffer[100]={0}, action[MAX_TASK_ACTION_LEN]={0};
	char *lineptr1;
	int idx, entry_num, index, disable, val, do_update=0;
	WLAN_TIMER_TASK_ENTRY_T entry;
	WLAN_TIMER_TASK_ENTRY_T target[2];
	int i, j, found=0, tmpvalue;
	...
	while ( fgets(line_buffer, sizeof(line_buffer), fp) ) {
		...
		{
			lineptr1 = line_buffer;
			//action, hit_idx, index, disable
			strncpy(action, strsep(&lineptr1,","), MAX_TASK_ACTION_LEN);
			idx = atoi(strsep(&lineptr1,","));
			index = atoi(strsep(&lineptr1,","));
			disable = atoi(strsep(&lineptr1,","));
			printf("[%s %d] action is %s, idx is %d, index is %d, disable is %d\n",__FUNCTION__,__LINE__,
				action, idx, index, disable);
		}

		//if(!strncmp(action, TIMER_TASK_STR_REBOOT, sizeof(TIMER_TASK_STR_REBOOT)-1)){
			//idx = atoi(line_buffer);

		//for just one time timer task command, set enable as 0.
		if(idx){
			memset(target, 0x00, sizeof(WLAN_TIMER_TASK_ENTRY_T)*2);		

			//find the entry by taskId
			for(i=1; i<=entry_num; ++i){
				memset(&entry, 0, sizeof(WLAN_TIMER_TASK_ENTRY_T));
				*((char *)&entry) = (char)i;
				apmib_get(MIB_TIMER_TASK_TBL, (void *)&entry);			
				if(atoi(entry.taskid) == idx){
					idx = i;
					found = 1;
					break;
				}
			...
			*((char *)&entry) = (char)idx;
			apmib_get(MIB_TIMER_TASK_TBL, (void *)&entry);
			memcpy(&target[0], &entry, sizeof(WLAN_TIMER_TASK_ENTRY_T));

			//process tasktimer command done just one time
			entry.enable = 0;
			entry.week = 0;
			
			memcpy(&target[1], &entry, sizeof(WLAN_TIMER_TASK_ENTRY_T));
			apmib_set(MIB_TIMER_TASK_MOD, (void *)&target);		
			do_update = 1;
		}
		//}else 
			if(!strncmp(action, TIMER_TASK_STR_HEALTHMODE, sizeof(TIMER_TASK_STR_HEALTHMODE)-1)){
			apmib_save_wlanIdx();
			//indexÎª0±íÊ¾all band£»1-4±íÊ¾2.4G£»5-8±íÊ¾5G
			for(i=0; i<NUM_WLAN_INTERFACE; i++){
				for(j=0; j<NUM_VWLAN_INTERFACE; j++){
					wlan_idx = i;
					vwlan_idx = j;
					if(index == 0){  //index?a0¡À¨ª¨º?all band
						//set WLAN_DISABLED as disable
						...
						apmib_get( MIB_WLAN_WLAN_DISABLED, (void *)&val);
						if(((tmpvalue == 1) && (index >= 1) && (index <= 4)) || ((tmpvalue == 2) && (index >= 5) && (index <= 8))){  //2.4G
							if(j == 0){
								if(val != disable){
									apmib_set(MIB_WLAN_WLAN_DISABLED, (void *)&disable);
									do_update = 1;
...
//mib_display_data_content(DEFAULT_SETTING, pMib, sizeof(APMIB_T));	
	...
	int Value = 0;
	apmib_set(MIB_SSID_VER, (void *)&Value);
	apmib_update(CURRENT_SETTING);

	printf("flash reset success!!\n");
...
if(0)
{
	...
	//				fprintf(stderr,"%s:%d\n",__FUNCTION__,__LINE__);
		...
		else if(!strcmp(argv[argNum], "default-check"))
			{
#ifdef DEFAULT_SETTING_CHECK
			int ret;
			apmib_init();
			apmib_get(MIB_IS_DEFAULT_SETTING,(void *)&ret);
			if(ret)
...
#endif
#if TIMER_TASK_SUPPORT
		else if( !strcmp(argv[argNum], "-file_task_timer") ) {
			if (argc==3) {			
				setTaskTimer(argv[argNum+1]);
			}else
				printf("Miss arguments [task_timer_file]!\n");
			return 0;
		}
#endif
...
#endif
#ifdef SBWC
		else if (!strcmp(mib, "SBWC_TBL"))
			APMIB_GET(MIB_SBWC_NUM, (void *)&num);
#endif
#ifdef TIMER_TASK_SUPPORT
		else if (!strcmp(mib, "TIMER_TASK_TBL"))
			APMIB_GET(MIB_TIMER_TASK_NUM, (void *)&num);
#endif
#ifdef CONFIG_ANDLINK_SUPPORT
		else if (!strcmp(mib, "RTL_LINK_AREA_TBL"))
			APMIB_GET(MIB_RTL_LINK_AREA_TBL_NUM, (void *)&num);
#endif
		if (config_area == HW_MIB_AREA)
			pTbl = hwmib_table;
...
*/
	...
	strcpy((char *)pMib->domainName,"Realtek");
		...
		pMib->webLanguage=SIMPLE_CHINESE;
#endif
...
#endif	
#if defined(CONFIG_CMCC)||defined(CONFIG_APP_BOA_HE_UI)
	pMib->ipAddr[0] = 192;
	pMib->ipAddr[1] = 168;
	pMib->ipAddr[2] = 8;
	pMib->ipAddr[3] = 1;
	...
	pMib->dhcpClientEnd[3] = 200;
        pMib->dhcpLeaseTime = 1440;
#ifdef CONFIG_RTL_8197F_GW
        pMib->opMode=GATEWAY_MODE;
#endif
#ifdef CONFIG_RTL_8197F_AP
        pMib->opMode=BRIDGE_MODE;
#endif
#ifdef CONFIG_RTL_8197F_RPT
        pMib->opMode=REPEATER_MODE;
#endif
#endif
...
strcpy(pMib->userName,"user");
strcpy(pMib->userPassword,"user");
strcpy(pMib->ntpServerIp1,"time.nist.gov");
pMib->ntp_sync_interval = 3600;
#endif
...
#ifdef SBWC
	pMib->wlan_sbwc_enable = 0;
	pMib->wlan_sbwc_num = 0;
#endif
#ifdef TIMER_TASK_SUPPORT
	pMib->timer_task_num = 0;
#endif
#ifdef CONFIG_ANDLINK_SUPPORT
	pMib->rtl_link_area_tbl_num = 0;
#endif
#if defined(CONFIG_APP_TR069)
...
#ifdef SBWC
	case WLAN_SBWC_ARRAY_T:
#endif
#ifdef TIMER_TASK_SUPPORT
	case WLAN_TIMERTASK_ARRAY_T:
#endif
#ifdef CONFIG_ANDLINK_SUPPORT
	case RTL_LINK_AREA_ARRAY_T:
#endif
#ifdef RTK_CAPWAP
...
#ifdef CONFIG_APP_TR069
#ifdef CHECK_IPV4_DHCP_OPTION
	else if ((type == X_CT_COM_DHCP_OPTION60_ARRAY_T) || (type == X_CT_COM_DHCP_OPTION125_ARRAY_T)){
		X_CT_COM_DHCP_OPTION_Tp pEntry=(X_CT_COM_DHCP_OPTION_Tp)array_val;
...
#endif
#ifdef SBWC
	else if(type == WLAN_SBWC_ARRAY_T){
		WLAN_SBWC_ENTRY_Tp pEntry = (WLAN_SBWC_ENTRY_Tp)array_val;
		sprintf((char *)buf, " MAC:%02x%02x%02x%02x%02x%02x",pEntry->sbwc_sta_mac[0], pEntry->sbwc_sta_mac[1], pEntry->sbwc_sta_mac[2],
															pEntry->sbwc_sta_mac[3], pEntry->sbwc_sta_mac[4], pEntry->sbwc_sta_mac[5]);
		sprintf(tmp1, " tx_limit:%d rx_limit:%d", pEntry->sbwc_sta_tx_limit, pEntry->sbwc_sta_rx_limit);
		strcat(buf, tmp1);
	}
#endif
#ifdef TIMER_TASK_SUPPORT
	else if(type == WLAN_TIMERTASK_ARRAY_T){
		WLAN_TIMER_TASK_ENTRY_Tp pEntry = (WLAN_TIMER_TASK_ENTRY_Tp)array_val;
		sprintf((char *)buf, " taskid:%s timeoffset:%d week:%d enable:%d action:%s timeoffset2:%d index:%d", 
			pEntry->taskid, pEntry->timeoffset, pEntry->week, pEntry->enable, pEntry->action, pEntry->timeoffset2, pEntry->index);
	}
#endif
#ifdef CONFIG_ANDLINK_SUPPORT
	else if(type == RTL_LINK_AREA_ARRAY_T){
		RTL_LINK_AREA_ENTRY_Tp pEntry = (RTL_LINK_AREA_ENTRY_Tp)array_val;
		sprintf((char *)buf, " area:%s ip_domain:%s port:%d",	pEntry->rtl_link_area_code, pEntry->rtl_link_area_ip_domain, pEntry->rtl_link_area_port);
	}
#endif
#ifdef VOIP_SUPPORT 
	else if (type == VOIP_T) {
...
#endif
#ifdef SBWC
	WLAN_SBWC_ENTRY_T sbwc_entry;
	memset(&sbwc_entry, 0, sizeof(WLAN_SBWC_ENTRY_T));
#endif
#ifdef TIMER_TASK_SUPPORT
...
#endif
#ifdef CONFIG_ANDLINK_SUPPORT
	RTL_LINK_AREA_ENTRY_T  rtl_link_area_entry;
	memset(&rtl_link_area_entry, 0, sizeof(RTL_LINK_AREA_ENTRY_T));
#endif
...
//#endif	// CONFIG_RTK_MESH && _MESH_ACL_ENABLE_
...
#ifdef SBWC
	case WLAN_SBWC_ARRAY_T:
		if (!strcmp(val[0], "add")) {//flash set WLAN_SBWC_TBL add [sta_mac] [tx_limit] [rx_limit]
			id = MIB_SBWC_ADD;
			...
			string_to_hex(val[1], sbwc_entry.sbwc_sta_mac, 12);

			if(atoi(val[2]) < 0 || atoi(val[3]) < 0){
				printf("Invalid tx/rx_limit!\n");
				return;
			}
			sbwc_entry.sbwc_sta_tx_limit = atoi(val[2]);
			sbwc_entry.sbwc_sta_rx_limit = atoi(val[3]);
		}
		else if (!strcmp(val[0], "del")) {//flash set WLAN_SBWC_TBL del [idx]
			id = MIB_SBWC_DEL;
			...
			int_val = atoi(val[1]);
			if ( !APMIB_GET(MIB_SBWC_NUM, (void *)&entryNum)) {
				printf("Get capPortalAllowtbl entry number error!");
				return;
			}
			if ( int_val > entryNum ) {
			...
			*((char *)&sbwc_entry) = (char)int_val;
			if ( !APMIB_GET(MIB_SBWC_TBL, (void *)&sbwc_entry)){
				printf("Get table entry error!");
				return;
			}
		}
		else if (!strcmp(val[0], "delall")) //flash set WLAN_SBWC_TBL delall
			id = MIB_SBWC_DELALL;

		value = (void *)&sbwc_entry;
		break;
#endif
#ifdef TIMER_TASK_SUPPORT
	case WLAN_TIMERTASK_ARRAY_T:
		if (!strcmp(val[0], "add")) {//flash set TIMER_TASK_TBL add [taskid] [timeoffset] [week] [enable] [action] [timeoffset2] [index]
			id = MIB_TIMER_TASK_ADD;
			...
			strncpy(timer_task_entry.taskid, val[1], sizeof(timer_task_entry.taskid));
			timer_task_entry.timeoffset = atoi(val[2]);
			timer_task_entry.week = atoi(val[3]);
			timer_task_entry.enable = atoi(val[4]);
			strncpy(timer_task_entry.action, val[5], sizeof(timer_task_entry.action));
			timer_task_entry.timeoffset2 = atoi(val[6]);
			timer_task_entry.index = atoi(val[7]);
		}
		else if (!strcmp(val[0], "del")) {//flash set TIMER_TASK_TBL del [idx]
			id = MIB_TIMER_TASK_DEL;
			...
			int_val = atoi(val[1]);
			if ( !APMIB_GET(MIB_TIMER_TASK_NUM, (void *)&entryNum)) {
				printf("Get capPortalAllowtbl entry number error!");
				return;
			}
			if ( int_val > entryNum ) {
			...
			*((char *)&timer_task_entry) = (char)int_val;
			if ( !APMIB_GET(MIB_TIMER_TASK_TBL, (void *)&timer_task_entry)){
				printf("Get table entry error!");
				return;
			}
		}
		else if (!strcmp(val[0], "delall")) //flash set TIMER_TASK_TBL delall
			id = MIB_TIMER_TASK_DELALL;

		value = (void *)&timer_task_entry;
		break;
#endif
#ifdef CONFIG_ANDLINK_SUPPORT
	case RTL_LINK_AREA_ARRAY_T:
		if (!strcmp(val[0], "add")) {//flash set RTL_LINK_AREA_TBL add [area] [ip/domain] [port]
			id = MIB_RTL_LINK_AREA_TBL_ADD;
			strncpy(rtl_link_area_entry.rtl_link_area_code, val[1], sizeof(rtl_link_area_entry.rtl_link_area_code));
			strncpy(rtl_link_area_entry.rtl_link_area_ip_domain, val[2], sizeof(rtl_link_area_entry.rtl_link_area_ip_domain));
			rtl_link_area_entry.rtl_link_area_port = atoi(val[3]);
			
		}
		else if (!strcmp(val[0], "del")) {//flash set RTL_LINK_AREA_TBL del [idx]
			id = MIB_RTL_LINK_AREA_TBL_DEL;				
			if ( valNum < 2 ) {
				printf("input argument is not enough!\n");
				return;
			}
			int_val = atoi(val[1]);
			if ( !APMIB_GET(MIB_RTL_LINK_AREA_TBL_NUM, (void *)&entryNum)) {
				printf("Get RTL_LINK_AREA_TBL entry number error!");
				return;
			}
			if ( int_val > entryNum ) {
				printf("Element number is too large!\n");
			...
			*((char *)&rtl_link_area_entry) = (char)int_val;
			if ( !APMIB_GET(MIB_RTL_LINK_AREA_TBL, (void *)&rtl_link_area_entry)){
				printf("Get table entry error!");
				return;
			}
		}
		else if (!strcmp(val[0], "delall")) //flash set RTL_LINK_AREA_TBL delall
			id = MIB_RTL_LINK_AREA_TBL_DELALL;

		value = (void *)&rtl_link_area_entry;
		break;	
#endif
	case BYTE13_T:
	...
	if(MIB_HW_NIC0_ADDR == id){
		...
		char mac_buf[18] = {0};
		strcpy(mac_buf, val[0]);
		char *ptr = NULL;
		ptr = mac_buf;
		char mac_buf_e[6][3] = {0};
		unsigned char mac[6] = {0};
		int i;
		for (i = 0; i < 6; i++)
		{
			memcpy(mac_buf_e[i], ptr+(2*i), 2);
		}

		for (i = 0; i < 6; i++)
		{
			mac[i] = (unsigned char )strtol(mac_buf_e[i], NULL, 16);
		}
		...
		if((unsigned char )(mac[5]+rise)==0x0){
			mac[4] += 1;
			if((unsigned char) mac[4]==0x0)
			mac[3] += 1;
		}

		char *wan_name[1] = {"HW_NIC1_ADDR"};
		sprintf(cmd_buf, "flash set %s %02x%02x%02x%02x%02x%02x",
				wan_name[0], mac[0], mac[1], mac[2], mac[3], mac[4], (unsigned char )(mac[5]+rise));
		printf("%s\n", cmd_buf);
		system(cmd_buf);
#endif
		...
		for (i = 0; i < sizeof(wl_names)/sizeof(char *); i++)
		{
			sprintf(cmd_buf, "flash set %s %02x%02x%02x%02x%02x%02x",
					wl_names[i], mac[0], mac[1], mac[2], (unsigned char )(mac[3]+(i*16)), mac[4], (unsigned char )(mac[5]+rise));
			if (0 == i)
				printf("%s\n", cmd_buf);
			system(cmd_buf);
		}
		...
		for (i = 0; i < sizeof(wl1_names)/sizeof(char *); i++)
		{
			sprintf(cmd_buf, "flash set %s %02x%02x%02x%02x%02x%02x",
					wl1_names[i], mac[0], mac[1], mac[2], (unsigned char )(mac[3]+(i*16)), mac[4], (unsigned char )(mac[5]+rise));
			if (0 == i)
				printf("%s\n", cmd_buf);
			system(cmd_buf);
		}
...
#endif	
#ifdef SBWC
		else if(pTbl[idx].id == MIB_SBWC_TBL)
			APMIB_GET(MIB_SBWC_NUM, (void *)&num);
#endif
#ifdef TIMER_TASK_SUPPORT
		else if(pTbl[idx].id == MIB_TIMER_TASK_TBL)
			APMIB_GET(MIB_TIMER_TASK_NUM, (void *)&num);
#endif
#ifdef CONFIG_ANDLINK_SUPPORT
		else if(pTbl[idx].id == MIB_RTL_LINK_AREA_TBL)
			APMIB_GET(MIB_RTL_LINK_AREA_TBL_NUM, (void *)&num);
#endif
...
#endif
#ifdef CONFIG_APP_MINIDLNA
		else if ( pTbl[idx].id == MIB_MINIDLNA_SHAREDFOLDER_TBL)
		{
			APMIB_GET(MIB_MINIDLNA_SHAREDFOLDER_TBL_NUM, (void *)&num);
		}
#endif
...
static int initWlan(char *ifname)
{
...
#ifdef SBWC
	char cmd[128] = {0};
	int sbwc_enable = 0, entryNum = 0, idx;
	WLAN_SBWC_ENTRY_T sbwc_entry;
#endif
	...
	apmib_get(MIB_WLAN_PEER_BSSID, (void *)mac);
	if ( mode==CLIENT_MODE && memcmp(mac, "\x00\x00\x00\x00\x00\x00", 6) && !(mac[0] & 0x01)){		
		memset(pmib->dot11StationConfigEntry.dot11DesiredBssid, 0, 6);
		memcpy(pmib->dot11StationConfigEntry.dot11DesiredBssid, mac, 6);	
...
#if 1//CONFIG_RTK_WPS_MESH_SUPPORT
...
#if 1//TARGETSSID_WEIGHT_SUPPORT
	apmib_get(MIB_WLAN_PREFER_BSSID, (void *)mac);
	if ( mode==CLIENT_MODE && memcmp(mac, "\x00\x00\x00\x00\x00\x00", 6) && !(mac[0] & 0x01)){		
		memset(pmib->miscEntry.prefer_bssid, 0, 6);
		memcpy(pmib->miscEntry.prefer_bssid, mac, 6);		
		printf("=== [%s %d]set prefer_bssid(%02x%02x%02x%02x%02x%02x),ifname(%s)\n",__FUNCTION__,__LINE__,mac[0],mac[1],mac[2],mac[3],mac[4],mac[5],ifname);		
	}
#endif

	apmib_get(MIB_DEF_GW_MAC, (void *)mac);
/*
	memset(pmib->dot11OperationEntry.dfgw_mac, 0, 6);
	memcpy(pmib->dot11OperationEntry.dfgw_mac, mac, 6);	
*/
...
#undef COPY_LUT_VAL
		...
		if (pmib->dot11RFEntry.dot11RFType == 10) { // Zebra
            ...
            if(intVal == 10)    //Andlink RadioConfig transmitPower 10%
                intVal = 20;
            else if(intVal == 20)   //Andlink RadioConfig transmitPower 20%
                intVal = 14;
            else if(intVal == 30)   //Andlink RadioConfig transmitPower 30%
                intVal = 10;
            else if(intVal == 40)   //Andlink RadioConfig transmitPower 40%
                intVal = 8;
            else if(intVal == 50)   //Andlink RadioConfig transmitPower 50%
                intVal = 6;
            else if(intVal == 60)   //Andlink RadioConfig transmitPower 60%
                intVal = 4;
            else if(intVal == 70)   //Andlink RadioConfig transmitPower 70%
                intVal = 3;
            else if(intVal == 80)   //Andlink RadioConfig transmitPower 80%
                intVal = 2;
            else if(intVal == 90)   //Andlink RadioConfig transmitPower 90%
                intVal = 1;
#endif				
			if (intVal) {
				for (i=0; i<14; i++) {
...
//!CONFIG_RTL_8196B => rtl8651c+rtl8190
    ...
    apmib_get(MIB_RTL_LINK_ENABLE, (void *)&intVal2);
    if(intVal2){
    	apmib_get(MIB_WLAN_STA_NUM, (void *)&intVal);
    	if(intVal){
            pmib->dot11StationConfigEntry.supportedStaNum = intVal;
    	}
    	apmib_get(MIB_WLAN_RSSI_TH, (void *)&intVal);
    	pmib->rlr_profile.RSSIThreshold = intVal;
    	apmib_get(MIB_RTL_LINK_ROAMING_TIME_GAP, (void *)&intVal);
    	if(intVal == 0)
    	    intVal = 1;
        pmib->rlr_profile.staRoamingTimeGap = intVal;
    	apmib_get(MIB_RTL_LINK_ROAMING_RSSI_GAP, (void *)&intVal);
    	pmib->rlr_profile.staRoamingRSSIGap = intVal;
    	apmib_get(MIB_RTL_LINK_ROAMING_SWITCH, (void *)&intVal);
    	pmib->rlr_profile.roaming_switch= intVal;
    	apmib_get(MIB_RTL_LINK_ROAMING_QOS, (void *)&intVal);
    	pmib->rlr_profile.roaming_qos= intVal;
        apmib_get(MIB_RTL_LINK_ROAMING_RETRY_RATIO, (void *)&intVal);
        pmib->rlr_profile.retry_ratio = intVal;
        apmib_get(MIB_RTL_LINK_ROAMING_FAIL_RATIO, (void *)&intVal);
        pmib->rlr_profile.fail_ratio = intVal;
        apmib_get(MIB_RTL_LINK_ROAMING_DISMISS_TIME, (void *)&intVal);
        pmib->sr_profile.block_aging = intVal;
    }
#endif
	...
	if(intVal & CAPWAP_ROAMING_ENABLE || intVal & CAPWAP_11V_ENABLE) {
...
&& encrypt < 7
...
#endif
#if 1//SSID_PRIORITY_SUPPORT
#ifdef CONFIG_CMCC
	apmib_get(MIB_WLAN_MANUAL_PRIORITY, (void *)&intVal);
	pmib->miscEntry.manual_priority = intVal;	
#endif
#endif
...
#ifdef SBWC
	if(!APMIB_GET(MIB_SBWC_ENABLE, (void *)&sbwc_enable)){
		printf("Get sbwc_enable error!\n");
		return;
	}
	if (sbwc_enable > 0){
		
		if ( !APMIB_GET(MIB_SBWC_NUM, (void *)&entryNum)) {
			printf("Get entryNum error!\n");
			return;
		}
		for(idx = 1; idx <= entryNum; idx++){
			*((char *)&sbwc_entry) = (char)idx;
			if ( !APMIB_GET(MIB_SBWC_TBL, (void *)&sbwc_entry)){
				printf("Get table entry error!\n");
				return;
			}
			snprintf((char *)cmd, sizeof(cmd)-1, "iwpriv %s sta_bw_control %02x%02x%02x%02x%02x%02x,%d,%d", ifname,
				sbwc_entry.sbwc_sta_mac[0], sbwc_entry.sbwc_sta_mac[1], sbwc_entry.sbwc_sta_mac[2],
				sbwc_entry.sbwc_sta_mac[3], sbwc_entry.sbwc_sta_mac[4], sbwc_entry.sbwc_sta_mac[5],
				sbwc_entry.sbwc_sta_tx_limit, sbwc_entry.sbwc_sta_rx_limit);
			system(cmd);
		}
	}
#endif
	...
	/* wifi accelerate */
	if(vwlan_idx==0){
		apmib_get(MIB_WLAN_QOS_ENHANCE, (void *)&intVal);
		snprintf((char *)buf1, sizeof(buf1), "iwpriv %s set_mib qos_enhance_enable=%d", ifname, intVal);		
		system((char *)buf1);
	}

#endif

/*record client hostname & ip*/
#ifdef CONFIG_RECORD_CLIENT_HOST
#if defined(CONFIG_APP_SDK_ONENET)
    snprintf((char *)buf1, sizeof(buf1), "iwpriv %s set_mib client_host_sniffer_enable=1", ifname);
    system((char *)buf1);    
#endif
#endif
