...
#endif
#if defined(CONFIG_APP_ARPTABLES)
char Arptables[]="arptables";
char _arp[] = "arp";
#endif
...
#if defined(CONFIG_APP_CMCC_ADAPTER)
...
#if defined (CONFIG_RTL_8198)|| defined (CONFIG_RTL_8198C) || defined (CONFIG_RTL_8367R_SUPPORT) || defined(CONFIG_RTL_8363NB_SUPPORT)
			...
			for (i=1; i<=QoS_Rule_EntryNum; i++) {
				...
				*((char *)&entry) = (char)i;
				...
				)
				{
					...
					#if defined(CONFIG_APP_CMCC_ADAPTER)
						if(entry.bandwidth == NO_RATE_LIMIT)
							sprintf(str_ceil, "%ldkbit",uplink_speed);
						else
					#endif
						sprintf(str_ceil, "%ldkbit",entry.bandwidth);
				...
				#if defined(CONFIG_APP_CMCC_ADAPTER)
					|| entry.bandwidth_downlink == NO_RATE_LIMIT
				#endif
					){/*DOWNlink*/
						//sip match
						ipaddr=((struct in_addr *)entry.remote_ip_start);
...
//dip match
						...
						#if defined(CONFIG_APP_CMCC_ADAPTER)
							if(entry.bandwidth_downlink == NO_RATE_LIMIT)
								sprintf(str_ceil, "%ldkbit",downlink_speed);
							else
						#endif
							sprintf(str_ceil, "%ldkbit",entry.bandwidth_downlink);
...
int setURLFilter(void)
{
	...
	//printf("%s:%d entryNum=%d\n",__FUNCTION__,__LINE__,entryNum);
		...
		if(entry.usrMode==0)
			...
			sprintf(tmp1, "%s I%x;", tmp1,ntohl(((struct in_addr *)entry.ipAddr)->s_addr));
...
int setDoS(unsigned long enabled, int op, char *pInterface)
{
	char cmdBuffer[500];
...
#ifdef SUPPORT_WINNUKE_ICMP_REDIRECTION
    if((enabled>>19) & 1) //winnuke
    {
        //printf("%s:%d===================enabled Winnuke==================\n",__FUNCTION__,__LINE__);
        RunSystemCmd(NULL_FILE, Iptables, ADD, FORWARD, in, pInterface, _protocol, _tcp, tcp_flags, MSS_FLAG3, MSS_FLAG3, dport, "137:139", jump, DROP, NULL_STR);
		RunSystemCmd(NULL_FILE, Iptables, ADD, FORWARD, in, pInterface, _protocol, _tcp, tcp_flags, MSS_FLAG3, MSS_FLAG3, dport, "113", jump, DROP, NULL_STR);
		RunSystemCmd(NULL_FILE, Iptables, ADD, FORWARD, in, pInterface, _protocol, _tcp, tcp_flags, MSS_FLAG3, MSS_FLAG3, dport, "53", jump, DROP, NULL_STR);
        
		RunSystemCmd(NULL_FILE, Iptables, ADD, INPUT, in, pInterface, _protocol, _tcp, tcp_flags, MSS_FLAG3, MSS_FLAG3, dport, "137:139", jump, DROP, NULL_STR);
		RunSystemCmd(NULL_FILE, Iptables, ADD, INPUT, in, pInterface, _protocol, _tcp, tcp_flags, MSS_FLAG3, MSS_FLAG3, dport, "113", jump, DROP, NULL_STR);
		RunSystemCmd(NULL_FILE, Iptables, ADD, INPUT, in, pInterface, _protocol, _tcp, tcp_flags, MSS_FLAG3, MSS_FLAG3, dport, "53", jump, DROP, NULL_STR);
    }
	if((enabled>>20) & 1) //ICMP_redirection
    {
        //printf("%s:%d===================enabled ICMP_Redirection====================\n",__FUNCTION__,__LINE__);
	    RunSystemCmd(NULL_FILE, Iptables, ADD, FORWARD, in, pInterface, _protocol, _icmp, icmp_type, "5", jump, DROP, NULL_STR);
		
		RunSystemCmd(NULL_FILE, Iptables, ADD, INPUT, in, pInterface, _protocol, _icmp, icmp_type, "5", jump, DROP, NULL_STR);
    }
#endif
return 0;
...
int setIpFilter(char *Interface_wanPhy)
{
	int entryNum=0, index;
	IPFILTER_T entry;
	char ipAddr[30];
	...
	char ipAddrEnd[32];
	char dstipAddr[32];
	char dstipAddrEnd[32];	
	char srcIpAddrRange[64];
	char dstIpAddrRange[64];
	char ipAddrRange[128];
	char action[32];
	char protocol[32];
	int mode;
	int input_dns = 0;
	...
	for(index=1; index <= entryNum ; index++) {
		...
		*((char *)&entry) = (char)index;
...
#if defined(IP_RANGE_CMCC_SUPPORT)
		if(entry.ipFilterMode != mode)
			continue;
		
#ifdef CONFIG_IPV6
		if(entry.ipVer==IPv4){
#endif	
		if(entry.ipFilterMode==0)	//IpFilterMode: 0-blacklist; 1-whitelist
			snprintf(action,sizeof(action), "%s", DROP);
		else
			snprintf(action,sizeof(action), "%s", ACCEPT);
		if(entry.ipFilterType==PROTO_BASED)
		{
			//actually, ipFilterMode must be blacklist here
			switch(entry.protoType)
			{
				case PROTO_ICMP:
					RunSystemCmd(NULL_FILE, Iptables, ADD, FORWARD, in, "br0", _protocol, _icmp, jump, action, NULL_STR);
					break;
				case PROTO_ARP:
#if defined(CONFIG_APP_ARPTABLES)
					RunSystemCmd(NULL_FILE, Arptables, ADD, INPUT, in, "br0", jump, action, NULL_STR);
					RunSystemCmd(NULL_FILE, Arptables, ADD, OUTPUT, jump, action, NULL_STR);
#endif
					break;
				case PROTO_ICMP_ARP:
					RunSystemCmd(NULL_FILE, Iptables, ADD, FORWARD, in, "br0", _protocol, _icmp, jump, action, NULL_STR);				
		...
		else
		{
			...
			if((entry.ipAddr[0]||entry.ipAddr[1]||entry.ipAddr[2]||entry.ipAddr[3]|| 
				entry.ipAddrEnd[0]||entry.ipAddrEnd[1]||entry.ipAddrEnd[2]||entry.ipAddrEnd[3]))
			{
				tmpStr = inet_ntoa(*((struct in_addr *)entry.ipAddr));
				snprintf(ipAddr, sizeof(ipAddr), "%s", tmpStr);
				tmpStr = inet_ntoa(*((struct in_addr *)entry.ipAddrEnd));
				snprintf(ipAddrEnd, sizeof(ipAddrEnd), "%s", tmpStr);
				snprintf(srcIpAddrRange, sizeof(srcIpAddrRange), "%s-%s", ipAddr, ipAddrEnd);
			}
			else
				snprintf(srcIpAddrRange, sizeof(srcIpAddrRange), "");

			if((entry.dstipAddr[0]||entry.dstipAddr[1]||entry.dstipAddr[2]||entry.dstipAddr[3]|| 
				entry.dstipAddrEnd[0]||entry.dstipAddrEnd[1]||entry.dstipAddrEnd[2]||entry.dstipAddrEnd[3]))
			{
				tmpStr = inet_ntoa(*((struct in_addr *)entry.dstipAddr));
				snprintf(dstipAddr, sizeof(dstipAddr), "%s", tmpStr);
				tmpStr = inet_ntoa(*((struct in_addr *)entry.dstipAddrEnd));
				snprintf(dstipAddrEnd, sizeof(dstipAddrEnd), "%s", tmpStr);
				snprintf(dstIpAddrRange, sizeof(dstIpAddrRange), "%s-%s",dstipAddr, dstipAddrEnd);
			}
			else
				snprintf(dstIpAddrRange, sizeof(dstIpAddrRange), "");

			if(!srcIpAddrRange[0] &&!dstIpAddrRange[0])
				continue;

		 	if(entry.protoType==PROTO_TCP)
			{
				if(srcIpAddrRange[0]&&dstIpAddrRange[0])
               	 	RunSystemCmd(NULL_FILE, Iptables, ADD, FORWARD, _protocol, _tcp, match, ip_range,src_rnage, srcIpAddrRange, dst_rnage, dstIpAddrRange, jump, action, NULL_STR);
				else if(srcIpAddrRange[0])
					RunSystemCmd(NULL_FILE, Iptables, ADD, FORWARD, _protocol, _tcp, match, ip_range,src_rnage, srcIpAddrRange, jump, action, NULL_STR);
				else if(dstIpAddrRange[0])
					RunSystemCmd(NULL_FILE, Iptables, ADD, FORWARD, _protocol, _tcp, match, ip_range,dst_rnage, dstIpAddrRange, jump, action, NULL_STR);
			}
			else if(entry.protoType==PROTO_UDP)
			{			
				if(srcIpAddrRange[0]&&dstIpAddrRange[0])
				{
					RunSystemCmd(NULL_FILE, Iptables, ADD, FORWARD, _protocol, _udp, match, ip_range,src_rnage, srcIpAddrRange, dst_rnage, dstIpAddrRange, jump, action, NULL_STR);
					RunSystemCmd(NULL_FILE, Iptables, ADD, INPUT, _protocol, _udp, dport, "53:53", match, ip_range,src_rnage, srcIpAddrRange, dst_rnage, dstIpAddrRange, jump, action, NULL_STR);
				}
				else if(srcIpAddrRange[0])
				{
					RunSystemCmd(NULL_FILE, Iptables, ADD, FORWARD, _protocol, _udp, match, ip_range,src_rnage, srcIpAddrRange, jump, action, NULL_STR);
					RunSystemCmd(NULL_FILE, Iptables, ADD, INPUT, _protocol, _udp, dport, "53:53", match, ip_range,src_rnage, srcIpAddrRange, jump, action, NULL_STR);
				}
				else if(dstIpAddrRange[0])
				{
					RunSystemCmd(NULL_FILE, Iptables, ADD, FORWARD, _protocol, _udp, match, ip_range,dst_rnage, dstIpAddrRange, jump, action, NULL_STR);
					RunSystemCmd(NULL_FILE, Iptables, ADD, INPUT, _protocol, _udp, dport, "53:53", match, ip_range,dst_rnage, dstIpAddrRange, jump, action, NULL_STR);
				}
			}
			else if(entry.protoType==PROTO_BOTH)	
			{
...
#endif	
#endif
#endif //CONFIG_RTL_FAST_FILTER

	}
#ifdef IP_RANGE_CMCC_SUPPORT
	if(mode==1)
	{
	    RunSystemCmd(NULL_FILE, Iptables, ADD, FORWARD, in, "br0", out, Interface_wanPhy, jump, DROP, NULL_STR);//add default filter rule to chain
	    //printf("%s.%d.RunsystemCmd add default rule!!!\n",__FUNCTION__,__LINE__);
	}
#endif
	return 0;
}

static unsigned int keep_input_macfilter = 0;
int setMACFilter(void)
{
	...
	char cmdBuffer[80];
	int mode;
	unsigned int white_num_count=0;
...
#if defined(CONFIG_APP_EBTABLES)&&defined(CONFIG_EBTABLES_KERNEL_SUPPORT)
		if(mode){
			RunSystemCmd(NULL_FILE, Ebtables, ADD, INPUT,_src, macEntry, jump, ACCEPT,NULL_STR);	
			RunSystemCmd(NULL_FILE, Ebtables, ADD, OUTPUT,_dest, macEntry, jump, ACCEPT,NULL_STR);	
		}else{
			if(keep_input_macfilter){
...
#ifdef CONFIG_IPV6
		if(mode){
			RunSystemCmd(NULL_FILE, Ip6tables, ADD, FORWARD, match, "mac" ,mac_src, macEntry, jump, ACCEPT, NULL_STR);
			RunSystemCmd(NULL_FILE, Ip6tables, ADD, INPUT, match, "mac" ,mac_src, macEntry, jump, ACCEPT, NULL_STR);
		}
		else{
			RunSystemCmd(NULL_FILE, Ip6tables, ADD, FORWARD, match, "mac" ,mac_src, macEntry, jump, DROP, NULL_STR);
			if(keep_input_macfilter){
				RunSystemCmd(NULL_FILE, Ip6tables, ADD, INPUT, match, "mac" ,mac_src, macEntry, jump, DROP, NULL_STR);
			}
		}
#endif
		if(mode){
			RunSystemCmd(NULL_FILE, Iptables, ADD, FORWARD, match, "mac" ,mac_src, macEntry, jump, ACCEPT, NULL_STR);
			RunSystemCmd(NULL_FILE, Iptables, ADD, INPUT, match, "mac" ,mac_src, macEntry, jump, ACCEPT, NULL_STR);
		}
		else{
	...
	if(mode&&white_num_count){
#if defined(CONFIG_RTL_FAST_FILTER)

#else
#if defined(CONFIG_APP_EBTABLES)&&defined(CONFIG_EBTABLES_KERNEL_SUPPORT)
		RunSystemCmd(NULL_FILE, Ebtables, ADD, INPUT,"-i" ,"br0",jump, DROP,NULL_STR);
		RunSystemCmd(NULL_FILE, Ebtables, ADD, OUTPUT,"-i" ,"br0",jump, DROP,NULL_STR);
#else
#ifdef CONFIG_IPV6	
		RunSystemCmd(NULL_FILE, Ip6tables, ADD, FORWARD,"-i","br0",jump, DROP, NULL_STR);
		RunSystemCmd(NULL_FILE, Ip6tables, ADD, INPUT,"-i","br0", jump, DROP, NULL_STR);
#endif
		RunSystemCmd(NULL_FILE, Iptables, ADD, FORWARD,"-i","br0",jump, DROP, NULL_STR);
		RunSystemCmd(NULL_FILE, Iptables, ADD, INPUT,"-i","br0", jump, DROP, NULL_STR);
#endif	
#endif
	}
	return 0;
...
void setRulesWithOutDevice(int opmode, int wan_dhcp , char* pInterface_wanPhy,char* Interface_wanPhy, char* pInterface)
{
...
#endif
#if defined(CONFIG_APP_ARPTABLES)
	RunSystemCmd(NULL_FILE,Arptables,FLUSH,NULL_STR);
	RunSystemCmd(NULL_FILE,Arptables,X,NULL_STR);
	RunSystemCmd(NULL_FILE,Arptables,Z,NULL_STR);
#endif
...
defined(CONFIG_RTL_HW_NAPT)
	...
	setDoS(dos_enabled, opmode, pInterface);
	if(wan_dhcp==PPTP){
...
#ifdef MULTI_PPPOE
	if(isNeedSetOnce)
		setRulesWithOutDevice(opmode,wan_dhcp,pInterface_wanPhy,Interface_wanPhy, pInterface);
#else
		setRulesWithOutDevice(opmode,wan_dhcp,pInterface_wanPhy,Interface_wanPhy, pInterface);
#endif
...
RunSystemCmd(NULL_FILE, Iptables, ADD, INPUT, _protocol, _udp,  dport, "1900:1900", in, pInterface, _dest, WanIpAddr, jump,DROP, NULL_STR);
    ...
    apmib_get(MIB_RTL_LINK_ENABLE, (void *) &intVal);
    if(intVal){
            //5683: coap port
            RunSystemCmd(NULL_FILE, Iptables, ADD, INPUT, _protocol, _udp,  dport, "5683:5683", in, pInterface_wanPhy, _dest, WanIpAddr, jump, ACCEPT, NULL_STR);
			RunSystemCmd(NULL_FILE, Iptables, ADD, INPUT, _protocol, _udp,  sport, "5683:5683", in, pInterface_wanPhy, _dest, WanIpAddr, jump, ACCEPT, NULL_STR);
			
			char *offset;
			char WanIpAddr_bc[30]={0};
			strcpy(WanIpAddr_bc, WanIpAddr);
			offset = strrchr(WanIpAddr_bc, '.');
			strcpy(offset, ".255");
            RunSystemCmd(NULL_FILE, Iptables, ADD, INPUT, _protocol, _udp,  dport, "5683:5683", in, pInterface_wanPhy, _dest, WanIpAddr_bc, jump, ACCEPT, NULL_STR);

#ifdef CONFIG_ANDLINK_IF5_SUPPORT
			int mqtt_port=0;
			char port_tmp[16];
			apmib_get(MIB_RTL_LINK_CGW_MQTT_PORT, (void *) &mqtt_port);
			snprintf(port_tmp, sizeof(port_tmp), "%d", mqtt_port);
			RunSystemCmd(NULL_FILE, Iptables, ADD, INPUT, _protocol, _tcp, in, pInterface_wanPhy, _dest, WanIpAddr, sport, port_tmp, jump, ACCEPT, NULL_STR);
#endif
	}
#endif
