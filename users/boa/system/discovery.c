...
int waitForOffer(int sock_fd, int timeout, unsigned char *myEth)
{
	...
	//move tv init out of while to make select continue to receive packet if not time out! 
	tv.tv_sec = timeout;
	tv.tv_usec = 0;
	...
	FD_SET(sock_fd, &readable);
		...
		{
			//printf("\n%s:%d####mac does not match!tv.tv_sec=%u tv.tv_usec=%u\n",__FUNCTION__,__LINE__,tv.tv_sec,tv.tv_usec);
			continue;
		}
		return 1;
	}
...
int createSock(char *wan_if, unsigned char *myEth, struct sockaddr_ll *ret_dest)
{
	int fd;
	struct ifreq ifr;
	struct sockaddr_ll dest, my_ethaddr;
	...
	my_ethaddr.sll_family = AF_PACKET;
	my_ethaddr.sll_protocol = htons(ETH_P_IP);

	strncpy(ifr.ifr_name, wan_if, sizeof(ifr.ifr_name));
	if (ioctl(fd, SIOCGIFINDEX, &ifr) < 0) 
		return -1;
		
	my_ethaddr.sll_ifindex = ifr.ifr_ifindex;	
	my_ethaddr.sll_halen = 6;
	my_ethaddr.sll_pkttype=PACKET_HOST;
	memcpy(my_ethaddr.sll_addr, myEth, 6);
	...
	if (bind(fd, (struct sockaddr *)&my_ethaddr, sizeof(struct sockaddr_ll)) < 0) 
	{		
		//close(fd);
		return -1;
	}	
...
int sendDiscovery(int sock_fd, struct dhcpPacket *dhcpPkt, unsigned char *myEth, struct sockaddr_ll *dest)
{
	...
	while (1)
	{
		...
		if(retval<=0 && padiAttempts>MAX_RETRY_NUM)
		{
			//printf("%s:%d####\n",__FUNCTION__,__LINE__);
			return -1;  
		}
