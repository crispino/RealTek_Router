...
void set_dhcp6s()
{
	...
	if(apmib_get(MIB_OP_MODE,(void*)&opMode)==0) {
		printf("get MIB_OP_MODE fail!\n");
		return;
	}

	if(opMode != GATEWAY_MODE) {
		printf("opMode not gateway, no need to start dhcp6s\n");
		return;
	}

	//IPv6 enabled check
	int ipv6_wanEnable = 0;
	if(apmib_get(MIB_IPV6_WAN_ENABLE,(void*)&ipv6_wanEnable)==0) {
	...
	int link_Type = 0;
	int pd_Enable = 0;
	if(!apmib_get(MIB_IPV6_ORIGIN_TYPE, &origin_Type)) {
		printf("get MIB_IPV6_ORIGIN_TYPE fail!\n");
		return;
	}
	
	if(!apmib_get(MIB_IPV6_LINK_TYPE, &link_Type)) {
		printf("get MIB_IPV6_LINK_TYPE fail!\n");
		return;
	}

    if(!apmib_get(MIB_IPV6_DHCP_PD_ENABLE,&pd_Enable)){
		printf("get MIB_IPV6_DHCP_PD_ENABLE fail\n");
		return;
	}

	if (origin_Type == 0 && link_Type == 0 && pd_Enable) {
		printf("pd enabled, no need to start dhcp6s\n");
		return;
	}
...
start_dhcp6s:
	...
	if(isFileExist(DHCP6S_PID_FILE)) {
		...
		if(dhcp6sCfgParam.enabled == 1) {
			printf("%s:%d, start dhcp6s, interface:%s................\n",
									__FUNCTION__, __LINE__, dhcp6sCfgParam.interfaceNameds);
			RunSystemCmd(NULL_FILE, "/bin/dhcp6s", dhcp6sCfgParam.interfaceNameds, NULL_STR);
		}
	}
...
void set_radvd()
{
	...
	if(opMode != GATEWAY_MODE) {
		printf("opMode not gateway, no need to start radvd\n");
		return;
	}
