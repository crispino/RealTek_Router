...
#define AUTO_DHCP_PID_FILE "/var/run/auto_dhcp.pid"
...
int main() 
{
	...
	int pid_fd, retval, sig;
	RUN_STATE auto_dhcp_state=RUN_INIT;
	pid_fd = pidfile_acquire(AUTO_DHCP_PID_FILE);
	...
	struct timeval timeout={MAX_TIME_OUT, 0}; //30s time out
	while(1)                 //read message from kernel
	{
		timeout.tv_sec = MAX_TIME_OUT;
		timeout.tv_usec = 0;
		...
		max_fd = (signal_pipe[0] > sock_fd ? signal_pipe[0] : sock_fd);		

		retval = select(max_fd + 1, &fds, NULL, NULL, &timeout);
		...
		{
			//printf("\ntime out! auto_dhcp_state=%d\n",auto_dhcp_state);
			if(auto_dhcp_state!=RUN_DHCPC_GET_IP)
			{
				//printf("\ncalling switch_dhcpc_dhcpd()\n");
				switch_dhcpc_dhcpd();
			}
		}
		else if(retval>0)
		{
			if(FD_ISSET(signal_pipe[0], &fds))
			{
				...
				{
					//printf("\nCould not read signal!\n");
					continue; 
				}
				switch (sig) 
				{
					case SIGUSR1: //dhcpc get ip						
						auto_dhcp_state=RUN_DHCPC_GET_IP;
						//printf("\nauto dhcp daemon receive signal SIGUSR1!\n");
						break;
					case SIGUSR2:  //dhcpc can't get ip
						auto_dhcp_state=RUN_DHCPC_NO_IP;
						//printf("\nauto dhcp daemon receive signal SIGUSR2!\n");
						switch_dhcpc_dhcpd();
						break;
				}
			}
