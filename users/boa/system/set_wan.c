...
//#ifdef TR181_V6_SUPPORT
	...
	// add syslog
	syslog(LOG_INFO, "wan dhcp connected, ip %s mask %s gw %s\n", wanip, mask, remoteip);
#if defined(CONFIG_DYNAMIC_WAN_IP)
	}
#endif
#if defined(CONFIG_DYNAMIC_WAN_IP)
...
//	apmib_get(MIB_WAN_DHCP,(void *)&wan_type);
	...
	//RunSystemCmd(NULL_FILE, "killall", "-9", "chkwan", NULL_STR);
#endif	
...
#ifdef REINIT_WAN_SUPPORT
void reinit_wan()
{
	wan_disconnect("all");

	int wan_type, op_mode;
	int wisp_wan_id;
	char wan_interface[16]={0}, lan_interface[16]={0};
	apmib_get(MIB_WAN_DHCP,(void *)&wan_type);
	apmib_get(MIB_OP_MODE,(void *)&op_mode);
	apmib_get(MIB_WISP_WAN_ID,(void *)&wisp_wan_id);
	...
	if(op_mode==WISP_MODE)
	{
		...
		strcpy(wan_interface, "eth1");

	strcpy(lan_interface, "br0");
	
	start_wan(wan_type, op_mode, wan_interface, lan_interface, wisp_wan_id, 1);
}
#endif
