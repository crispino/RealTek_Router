...
///////////////////////////////////////
	...
	if((opmode ==BRIDGE_MODE) && (wlan0_mode == 1 || wlan0_mode == 0)){ //when wlan is client mode or ap mode, user change lan setting
		check_flag=1;
	}
...
void formWanTcpipSetup(request *wp, char *path, char *query)
{
    ...
    int configured = 1;
    apmib_set(MIB_WAN_CONFIGURED, (void *)&configured);
#endif
...
int dhcpRsvdIp_List(request *wp, int argc, char **argv)
{
	...
	nBytesSent += req_format_write(wp, ("<tr class=\"tbl_head\">"
      	"<td align=center width=\"25%%\" ><font size=\"2\"><b><script>dw(tcpip_dhcp_ipaddr)</script></b></font></td>\n"
#ifdef SUPPORT_DHCP_PORT_IP_BIND
...
int checkClientConnType(char *client_mac, int connType)
{
	...
	if((connType==WIRED_CONN && ethernet_type==1) || (connType==WIRELESS_CONN && ethernet_type==0))
		return 1;
	else 
		return 0;	
}
...
int getDhcpWiredClientList(request *wp, int argc, char **argv)
{
	FILE *fp;
	int nBytesSent=0;
	int element=0, ret;
	...
	while (1) {
		...
		nBytesSent += req_format_write(wp,
            ("<ul id = 'display%d' style='display:'><li class='l1'><span id = 'dhcpmac%d' class='device%d'>%s</li><li class='l2'>%s</li><li class='l3'>%s</li><li class='l6'>无</li>"),
            element,element, device_type,macAddr, ipAddr, hostname);
...
int local_getGWIp(char *gwip)
{
    char ifname[16]={0}, cmd_buf[128]={0};
    struct in_addr ipaddr;

	system("ifconfig eth1 0.0.0.0");
	snprintf(ifname, sizeof(ifname), "%s", "br0");

    if(getDefaultRoute(ifname, (void *)&ipaddr)) {
        snprintf(gwip, 16, inet_ntoa(ipaddr));
        sprintf(cmd_buf, "ping %s -c1 > /dev/null 2>&1", gwip);   //bridge mode, add gw info in arp table
        system(cmd_buf);
        return 0;
    } else {        
        return -1;
...
int getWiredClientList(request *wp, int argc, char **argv)
{
	int i, element, nBytesSent;
    LAN_DEV_BASE_INFO lan_sta[MAX_STA_NUM];
    int connect_num;
    char gw_mac[18]={0}, cur_ip[16]={0}, gw_ip[16] = {0};
    char tmp_mac[18], line_buffer[512];
	char macAddr[18] = {0}, ipAddr[16], hostname[128] = {0};
    FILE *fp=NULL;
    char *pchar;
    int wan_port=-1;
	int device_type = 5;
	...
	element = nBytesSent = 0;
    for(i=0; i<connect_num; i++){
        if(lan_sta[i].conn_port == wan_port)    //skip wan port
            continue;

        sprintf(macAddr, "%02X:%02X:%02X:%02X:%02X:%02X", 
            lan_sta[i].mac[0], lan_sta[i].mac[1], lan_sta[i].mac[2], lan_sta[i].mac[3],
            				lan_sta[i].mac[4], lan_sta[i].mac[5]);

        snprintf(hostname, sizeof(hostname), "%s", lan_sta[i].dev_name);
        snprintf(ipAddr, sizeof(ipAddr), "%s", inet_ntoa(*((struct in_addr *)&lan_sta[i].ip)));
		
		if (!strcmp(ipAddr, "0.0.0.0")) //skip invalid ip address 
			continue;
...
int dhcpWiredClientList(request *wp, int argc, char **argv)
{
	...
	if (opmode == GATEWAY_MODE) {
		return getDhcpWiredClientList(wp, argc, argv);
	} else {
		return getWiredClientList(wp, argc, argv);
	}
}


int dhcpMacCloneClientList(request *wp, int argc, char **argv)
{
	FILE *fp;
	int nBytesSent=0;
	int element=0, ret;
	...
	while (1) {
		...
		nBytesSent += req_format_write(wp,
			("<li onclick=\"pick('%s','macCloneMac','test1')\" >%s</li>"),
			macAddr, ipAddr);
...
int getDhcpWirelessClientList(request *wp, int argc, char **argv)
{
	FILE *fp;
	int nBytesSent=0;
	int element=0, ret;
	...
	while (1) {
		...
		if(argv[0]==NULL)
		nBytesSent += req_format_write(wp,
            ("<ul id = 'display%d' style='display:'><li class='l1'><span id = 'mac%d' class='device%d'>%s</li><li class='l2'>%s</li><li class='l3'>%s</li><li class='l6'><em id = 'emid_%d' style='display:' onclick=\"disconnect('mac%d','emid_%d','Ocemid_%d');\">加入黑名单</em><em id ='Ocemid_%d' style='display:none'>正在断开</em></li></ul>"),
            element,element,device_type,macAddr, ipAddr, hostname,element,element,element,element,element);
...
int getWirelessClientList(request *wp, int argc, char **argv)
{
	int i, element =0;
	int device_type=2;
	char macAddr[18] = {0};
	...
	for(i=0; i<connect_num; i++){
		...
		sprintf(macAddr, "%02X:%02X:%02X:%02X:%02X:%02X", 
				wlan_sta[i].mac[0], wlan_sta[i].mac[1], wlan_sta[i].mac[2], 
				wlan_sta[i].mac[3], wlan_sta[i].mac[4], wlan_sta[i].mac[5]);
		snprintf(hostname, sizeof(hostname), "%s", wlan_sta[i].dev_name);
		snprintf(ipAddr,sizeof(ipAddr),"%s",inet_ntoa(*((struct in_addr *)&wlan_sta[i].ip)));
...
int dhcpWirelessClientList(request *wp, int argc, char **argv)
{
	...
	if (opmode == GATEWAY_MODE) {
		return getDhcpWirelessClientList(wp, argc, argv);
	} else {
		return getWirelessClientList(wp, argc, argv);
	}
}
...
{
	char *submitUrl=NULL,*strVal=NULL, *serName=NULL,*username=NULL,*password=NULL;
	char erroMsgBuf[256]={0};
	int intVal=0,wanDhcp=0;
	...
	if(strVal[0]){
		...
		{
			wanDhcp=DHCP_CLIENT;
			if(!apmib_set(MIB_WAN_DHCP,(void*)&wanDhcp)){
				snprintf(erroMsgBuf,sizeof(erroMsgBuf),"set MIB_WAN_DHCP fail!\n");
				goto setformFastConf_end;
			}
		}else if(intVal==2)//pppoe
		{
			wanDhcp=PPPOE;
			username=req_get_cstream_var(wp, "username", "");
			if(!username[0])
			{
			...
			serName=req_get_cstream_var(wp, "serviceName", "");
			
			if(!apmib_set(MIB_WAN_DHCP,(void*)&wanDhcp)){
				snprintf(erroMsgBuf,sizeof(erroMsgBuf),"set MIB_WAN_DHCP fail!\n");
				goto setformFastConf_end;
			}
			if(!apmib_set(MIB_PPP_USER_NAME,(void*)username)){
				snprintf(erroMsgBuf,sizeof(erroMsgBuf),"set MIB_PPP_USER_NAME fail!\n");
				goto setformFastConf_end;
			}
			if(!apmib_set(MIB_PPP_PASSWORD,(void*)password)){
				snprintf(erroMsgBuf,sizeof(erroMsgBuf),"set MIB_PPP_PASSWORD fail!\n");
				goto setformFastConf_end;
			}
			if(!apmib_set(MIB_PPP_SERVICE_NAME,(void*)serName)){
				snprintf(erroMsgBuf,sizeof(erroMsgBuf),"set MIB_PPP_SERVICE_NAME fail!\n");
				goto setformFastConf_end;
			}
		}
	}
