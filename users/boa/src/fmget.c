...
extern char *JSVersion;	// defined in version.c
...
int cmcc_init_wizard(request *wp)
{
	int ipmodeIdx=0;
	int opmode=0;
	apmib_get( MIB_OP_MODE, (void *)&opmode);
	if(opmode==BRIDGE_MODE) return 1;
	
#if 0 
def CONFIG_AUTO_DHCP_CHECK
	if(discovery_dhcp(wanIfname)==1){
		connTypeIdx=0;
		ipmodeIdx=0;
...
#endif
	//printf("%s:%d wanIfname=%s\n",__FUNCTION__,__LINE__,wanIfname);
	req_format_write(wp,"ipmodeIdx=%d;\n",ipmodeIdx);
	return 0;
}
...
int getInfo(request *wp, int argc, char **argv)
{
	...
	char lanip[64]={0},lanip_1[64]={0},lanip_2[64]={0},lanip_mask[24]={0};			
...
#ifdef CONFIG_APP_CMCC_GW_UI
	else if(!strcmp(name,("initPageWizard"))){
			cmcc_init_wizard(wp);
			return 0;
	}
#endif
...
#endif  
#if 0 
def CONFIG_APP_BOA_HE_UI
	else if(!strcmp(name,"ConMode"))
	{
	...
	else if(!strcmp(name,"isNeedWizard"))
	{
		...
		if(isNeedNoWizard)
			return req_format_write(wp, "%d",2);
		else
			return req_format_write(wp, "%d",1);
	}
...
//printf("\r\n buffer[%s],__[%s-%u]\r\n",buffer,__FILE__,__LINE__);			
	...
	else if(!strcmp(argv[0],("mesh_jscomment_end")))
	{
		...
		if ( getInAddr(BRIDGE_IF, IP_ADDR, (void *)&intaddr ) )
			{					
				...
				printf("UI get ip:%s %s\n",lanip_1,lanip_2);
				if(strcmp(lanip,lanip_2)==0)
					return req_format_write(wp, "%s", lanip_1 );
				else
		...
		if ( !apmib_get( MIB_IP_ADDR,  (void *)buffer) )
			...
			sprintf(lanip,"%s",inet_ntoa(*((struct in_addr *)buffer)));
			sscanf(lanip,"%[^.].%[^.].%[^.].%s",lanip0,lanip1,lanip2,lanip3);
			sprintf(lanip_1,"%s.%s.%s.1",lanip0,lanip1,lanip2);
			sprintf(lanip_2,"%s.%s.%s.253",lanip0,lanip1,lanip2);
			printf("UI get ip-rom:%s %s\n",lanip_1,lanip_2);
		...
		if ( !apmib_get( MIB_IP_ADDR,  (void *)buffer) )
			...
			sprintf(lanip,"%s",inet_ntoa(*((struct in_addr *)buffer)));
			sscanf(lanip,"%[^.].%[^.].%[^.].%s",lanip0,lanip1,lanip2,lanip3);
			sprintf(lanip_mask,"%s.%s.%s",lanip0,lanip1,lanip2);
			
             return req_format_write(wp, "%s", lanip_mask );
	...
	*/
	...
	/* Advance setting stuffs */
		...
		int val;
   		if ( apmib_get( MIB_PROVINCIAL_CODE, (void *)&val) && JIANGSU == val)
			strcat(buffer, JSVersion );
   		return req_format_write(wp, buffer);
	}
	// added by rock /////////////////////////////////////////
...
// for ap package of new ui , end
	...
	else if ( !strcmp(name, "wan-ip"))
  {
	...
	else if(!strcmp(name, "pocketRouter_Mode_countdown")) // 0:non-pocketRouter; 3: Router; 2:Bridge AP; 1:Bridge Client
	{
		...
		if(intVal == BRIDGE_MODE) //opmode is bridge
		{
...
// add for ds-lite
	...
	else if(!strcmp(name,"show_andlink"))
	{
...
#ifdef CONFIG_ANDLINK_SUPPORT	
	else if(!strcmp(name,"andlink_local_ip_addr"))
	{
		memset(buffer,0x00,sizeof(buffer));
		apmib_get( MIB_RTL_LINK_LOCAL_IP_ADDR,  (void *)buffer);
		return req_format_write(wp, "%s", inet_ntoa(*((struct in_addr *)buffer)) );
	}
	else if(!strcmp(name,"andlink_version"))
	{
		memset(buffer,0x00,sizeof(buffer));
		apmib_get(MIB_RTL_LINK_ANDLINK_VER,(void *)buffer);
		return req_format_write(wp, "%s", buffer);
	}
	else if(!strcmp(name,"andlink_heartbeat_port"))
	{
		memset(buffer,0x00,sizeof(buffer));
		apmib_get(MIB_RTL_LINK_HBPORT,(void *)buffer);
		return req_format_write(wp, "%s", buffer);

	}
	else if(!strcmp(name,"andlink_connect_gw_timeout"))
	{
		int gw_timeout;
		apmib_get(MIB_RTL_LINK_CONNECT_GW_TIMEOUT,(void *)&gw_timeout);
		sprintf(buffer, "%d", gw_timeout);
		return req_format_write(wp, "%s", buffer);
	}
	else if(!strcmp(name,"andlink_get_ip_timeout"))
	{
		int ip_timeout;
		apmib_get(MIB_RTL_LINK_GETIP_TIMEOUT,(void *)&ip_timeout);
		sprintf(buffer, "%d", ip_timeout);
		return req_format_write(wp, "%s", buffer);
	}
	else if(!strcmp(name,"andlink_auto_dhcp"))
	{
		apmib_get(MIB_RTL_LINK_AUTO_DHCP,(void *)&intVal);
		sprintf(buffer, "%d", intVal);
		return req_format_write(wp, "%s", buffer);
	}	
#endif
...
int get_macfilter_white_table_num(void)
{
	MACFILTER_T entry;
	int entryNum,num_count=0,index;
...
int getIndex(request *wp, int argc, char **argv)
{
	...
	else if ( !strcmp(name, "ipFilterMode")) {
	...
	else if ( !strcmp(name, "macFilterWhiteTableNum")) {
		val = get_macfilter_white_table_num();
		sprintf(buffer, "%d", val);
		req_format_write(wp, buffer);
		return 0;
	}
	else if ( !strcmp(name, "macFilterMode")) {
...
//		char tmpStr[10];
	...
	else if ( !strcmp(name, "isSyncVxdToRoot")) {
#if defined(SYNC_VXD_TO_ROOT) && defined(UNIVERSAL_REPEATER)		
		unsigned int wlanMode, rpt_enable;
	...
	else if ( !strcmp(name, "isCMCCSupport")) {
#ifdef CONFIG_CMCC
		snprintf(buffer, 50, "%d", 1);
#else
		snprintf(buffer, 50, "%d", 0);
#endif
		req_format_write(wp, buffer);
	}	
#ifdef CONFIG_CMCC
	else if(!strcmp(name, "ssid_pri"))
	{		
		if ( !apmib_get( MIB_WLAN_MANUAL_PRIORITY,	(void *)&val) )
			return -1;
...
#endif
	else if ( !strcmp(name, "isCMCC_AP_RPT")) {
#if defined(CONFIG_CMCC) && defined(CONFIG_RTL_AP_PACKAGE) 
		snprintf(buffer, 50, "%d", 1);
#else
		snprintf(buffer, 50, "%d", 0);
#endif
...
//for 11N
		...
		if(val == 1)
		{
			//3: bandwidth is 40M and 20/40 coexist is enabled
...
//### end
	...
	else if(!strcmp(name,"rtl8363NB")){
		#ifdef CONFIG_RTL_8363NB_SUPPORT
			req_format_write(wp, "1");
		#else
			req_format_write(wp, "0");
		#endif
...
/*	else if ( !strcmp(name, "StorageAnonAccessFtpEnable")) {
	...
	else if( !strcmp(name,"supportSuperUser"))
	{
		...
		else if(!strcmp(name,"subMenuOfHome")){		
#ifdef CONFIG_CMCC
#if defined(CONFIG_RTL_AP_PACKAGE) 
	sprintf(buffer,"%s", "sub_menu_wlan.htm");
#else
	sprintf(buffer,"%s", "sub_menu_setup.htm");	
#endif
#else
	sprintf(buffer,"%s", "sub_menu_setup.htm");
#endif
		...
		else if(!strcmp(name,"contentOfHome")){		
#ifdef CONFIG_CMCC
#if defined(CONFIG_RTL_AP_PACKAGE) 
	sprintf(buffer,"%s", "sub_menu_wlan.htm");
#else
	snprintf(buffer, 50, "%s", "opmode.htm");
#endif			
#else
		snprintf(buffer, 50, "%s", "wizard.htm");
#endif
		req_format_write(wp, buffer);	
		return 0;
	}
#ifdef CONFIG_ANDLINK_SUPPORT
	else if(!strcmp(name,"andlink_enable"))
	{
		int enabled;
		apmib_get(MIB_RTL_LINK_ENABLE,(void *)&enabled);
		if(enabled==1)
            sprintf(buffer, "%d", 1);
        else
            sprintf(buffer, "%d", 0);
	...
	else if(!strcmp(name,"andlink_mode"))
	{
		int mode;
		apmib_get(MIB_RTL_LINK_MODE,(void *)&mode);
        sprintf(buffer, "%d",(int)mode);
		req_format_write(wp, buffer);
		return 0;		
	}
	else if(!strcmp(name,"andlink_sync"))
	{
		int sync;
		apmib_get(MIB_RTL_LINK_SEARCH_ASYNC,(void *)&sync);
        sprintf(buffer, "%d",(int)sync);
		req_format_write(wp, buffer);
		return 0;		
	}
	else if(!strcmp(name,"andlink_recover"))
	{
		int recover;
		apmib_get(MIB_RTL_LINK_NEED_RECOVERY,(void *)&recover);
        sprintf(buffer, "%d",(int)recover);
		req_format_write(wp, buffer);
		return 0;		
	}
#endif
...
#if defined(CONFIG_DOMAIN_NAME_QUERY_SUPPORT)
        if(((operation_mode==BRIDGE_MODE) && (val==0 ||val==1)) || (operation_mode==0)){
	       if(lan_dhcp_mode == 0){
