...
{ /* if countDownPage.htm had been sent to browser, don't allow browser to get other page(br0 down) */
	...
	/* HE UI for repeater sitesurvey - by tesia */
	// apscan.asp?wlan_idx=1	
	//printf("######req->request_uri:%s, req->client_stream:%s\n", req->request_uri, req->client_stream);
	if (strstr(req->request_uri, "apscan.asp")) {
		char *strWlanId1, *ptr1;
		if (ptr1 = strstr(req->client_stream, "?wlan_idx=")) {
			*ptr1 = '\0';
			strWlanId1 = ptr1 + strlen("?wlan_idx=");
			extern int wlan_idx_to_scan;
			wlan_idx_to_scan = atoi(strWlanId1);
			//printf("#### strWlanId1:%s, wlan_idx_to_scan:%d\n", strWlanId1, wlan_idx_to_scan);
		}
	}
#endif
...
#else
#ifdef CONFIG_APP_BOA_HE_UI
//	fprintf(stderr, "###[%s %d] req->request_uri=%s not find!\nredirect to login###\n", __FUNCTION__, __LINE__, req->request_uri);
	send_redirect_perm(req,"login.html");
#else
