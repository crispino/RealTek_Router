...
/*
  * Input: phyport , int, 0~6
  * return value:  0 -linkdown, 1 -linkup,  other - error
  */
int checkLink(int phyport)
{
    int count=0;
    int linkstatus=-1;
    char buffer[128];
    FILE *fp;
    if(phyport > 6 || phyport < 0) {
        printf("port number %d error\n",phyport);
		return -1;
    }	
	if(!isFileExist("/proc/rtl865x/port_status")) {
		printf("/proc/rtl865x/port_status not exist\n");
		return -1;
	}	
	snprintf(buffer,sizeof(buffer),"cat /proc/rtl865x/port_status | grep \"Link\" > %s",ETH_LINK_STATUS_FILE);
	system(buffer);
    fp=fopen(ETH_LINK_STATUS_FILE,"r");
    if(fp==NULL) {
        printf("open link file failed\n");
        return linkstatus;
    }
    while(fgets(buffer,sizeof(buffer),fp))
    {
        //printf("the %d line is:%s",count,buffer); 
        if(count == phyport){
            if(strstr(buffer,"LinkDown")) {
                linkstatus = 0;
            } else if(strstr(buffer,"LinkUp")) {
                linkstatus = 1;
            }
            break;
        }
    ...
    fclose(fp);
	unlink(ETH_LINK_STATUS_FILE);
    return linkstatus;
}

/*
  * return value :  1 - can access internet; 0 - can not access internet
  */
int checkWanping(char *server1, char *server2, char *server3)
{
	...
	snprintf(buffer,sizeof(buffer),"ping %s -c 1",server1);
    ret=system(buffer);
	if(ret) {		
		return 0;
	}	
	
	snprintf(buffer,sizeof(buffer),"ping %s -c 1",server2);
    ret=system(buffer);
	if(ret) {		
		return 0;
	}	
	
	snprintf(buffer,sizeof(buffer),"ping %s -c 1",server3);
    ret=system(buffer);
	if(ret) {		
		return 0;
	}	
...
int getBuildImageTime(char *time, int len)
{
	FILE *fp=NULL;
	char *pchar=NULL;
	char buffer[512]={0}, tmp_time[64]={0};
...
int get_l2_mac_list(char *filename, unsigned char maclist[][6])
{
	if(filename==NULL)
		return 0; 
	...
	{			
		...
		if(strstr(line_buffer, "ff:ff:ff:ff:ff:ff") || strstr(line_buffer, "CPU") || strstr(line_buffer, "FID:1") || strstr(line_buffer, "mbr(8"))
			continue;	
...
int get_dhcp_client_list(struct dhcp_client_info *pclient, int array_len, int *real_num)
{	
	...
	FILE *fp=NULL;
	int i=0, ret=0;
	char *buf=NULL, tmpBuf[100]={0};	
	int dhcp_client_num=0;
	
	struct dhcpOfferedAddr *pDhcpEntry = NULL;
	...
	if ( stat("/var/lib/misc/udhcpd.leases", &status) < 0 )
		goto err;

	fileSize=status.st_size;

	dhcp_client_num=fileSize / sizeof( struct dhcpOfferedAddr );
	...
	if ( buf == NULL )
		goto err;
	fp = fopen("/var/lib/misc/udhcpd.leases", "r");
	if ( fp == NULL )
		goto err;
	...
	{		
		pDhcpEntry = (struct dhcpOfferedAddr *)(buf + i * sizeof( struct dhcpOfferedAddr ));

		pclient[i].ip=pDhcpEntry->yiaddr;
		memcpy(pclient[i].mac, pDhcpEntry->chaddr, 6);
		pclient[i].expires=pDhcpEntry->expires;
		strcpy(pclient[i].hostname, pDhcpEntry->hostname);	
	}
...
int get_wired_dhcp_client_list(struct dhcp_client_info *pWiredClient, int array_len, int *real_num)
{
	if(pWiredClient==NULL || array_len<1 || real_num==NULL)
		return 0;
	
	int dhcp_client_num=0, l2_tab_num=0;
	int i, j, idx=0;
	struct dhcp_client_info dhcp_client[MAX_STA_NUM];
	unsigned char maclist[MAX_STA_NUM][6];
	get_dhcp_client_list(dhcp_client, MAX_STA_NUM, &dhcp_client_num);

	l2_tab_num=get_l2_mac_list("/proc/rtl865x/l2", maclist);
	...
	for(i=0;i<dhcp_client_num;i++)
	{
		...
		for(j=0;j<l2_tab_num;j++)
		{
			if(memcmp(dhcp_client[i].mac, maclist[j], 6)==0)
			{
				memcpy(&pWiredClient[idx], &dhcp_client[i], sizeof(struct dhcp_client_info));
				//printf("\n%s:%d pWiredClient[%d].mac=%02X:%02X:%02X:%02X:%02X:%02X\n",__FUNCTION__,__LINE__,idx,pWiredClient[idx].mac[0],pWiredClient[idx].mac[1],pWiredClient[idx].mac[2],pWiredClient[idx].mac[3],pWiredClient[idx].mac[4],pWiredClient[idx].mac[5]);
				idx++;
				break;
			}
		}
