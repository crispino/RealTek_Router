...
void formFilter(request *wp, char *path, char *query)
{
	...
	char *strEndIpAddr, *strDstIpAddr, *strDstEndIpAddr, *strIpProtocol, *strProtoProtocol, *filterMode, *filterType;
	int type;
#endif
	...
	char *strDelAllUrl,*strUrlMode,*strMacMode,*strUsrMode;
	...
	if (strAddIp[0]) {
		...
		strIpProtocol = req_get_cstream_var(wp, "protocol", "");
		strProtoProtocol = req_get_cstream_var(wp, "proto_protocol", "");
		
		filterType = req_get_cstream_var(wp, ("ipFilterType"), "");
		if ( !strcmp(filterType, "on"))
...
#ifdef IP_RANGE_CMCC_SUPPORT
			&&(!strEndIpAddr[0])&&(!strDstIpAddr[0])&&(!strDstEndIpAddr[0])&&(strIpProtocol[0]=='0')&&(strProtoProtocol[0]=='9')
#endif
#ifdef CONFIG_IPV6
			&& !strIP6[0]
#endif
		...
		if (!strVal[0]
#ifdef IP_RANGE_CMCC_SUPPORT
			&&!strEndIpAddr[0]&&!strDstIpAddr[0]&&!strDstEndIpAddr[0]&&(ipEntry.ipFilterType==IP_BASED)
#endif
#ifdef CONFIG_IPV6
			&& !strIP6[0]
#endif
		...
		{		
			...
			{
		  	    strcpy(tmpBuf, ("<script>dw(ipfilter_type_error)</script>"));
				goto setErr_filter;
			}
			...
			else if(!strVal[0]&&!strEndIpAddr[0])
			{
				strVal="0.0.0.0";
			...
			else if(!strDstIpAddr[0]&&!strDstEndIpAddr[0])
			{
		...
		{
			//no ip address, only filter protocol
			if(
		#ifdef CONFIG_IPV6
				ipEntry.ipVer==IPv4&&
		#endif
				ipEntry.ipFilterType!=PROTO_BASED
				)
			{
		  		strcpy(tmpBuf, ("<script>dw(ipfilter_type_error)</script>"));
				goto setErr_filter;
	...
	if (strAddIp[0]) {
...
#endif
		{
			...
			if (strVal[0]) {
				if ( strVal[0] == '0' ) {
					...
					strcpy(tmpBuf, ("<script>dw(ipfilter_invalid_proto)</script>"));
					goto setErr_filter;
				}
			}
			else {
				strcpy(tmpBuf, ("<script>dw(ipfilter_proto_empty)</script>"));
				goto setErr_filter;
			}
		...
		else
		{
			...
			strVal = req_get_cstream_var(wp, ("proto_protocol"), "");
			...
			*((char *)&ipentrytmp) = (char)j;
			if ( apmib_get(MIB_IPFILTER_TBL, (void *)&ipentrytmp))
			{
				...
				if (((*((unsigned int*)ipentrytmp.ipAddr)) == (*((unsigned int*)ipEntry.ipAddr)))&&
					...
					(ipentrytmp.ipFilterMode==ipEntry.ipFilterMode)
				#ifdef CONFIG_IPV6
					&&(ipentrytmp.ipVer == IPv4)&&(ipEntry.ipVer == IPv4)
				#endif
				)
				{
						strcpy(tmpBuf, ("<script>dw(ipfilter_rule_exist)</script>"));
			...
			#elif defined(RTL_IPFILTER_SUPPORT_IP_RANGE)
				if (strEndIpAddr[0])
				{
					if (((*((unsigned int*)ipentrytmp.ipAddr)) == (*((unsigned int*)ipEntry.ipAddr)))&&
						((*((unsigned int*)ipentrytmp.ipAddrEnd))==(*((unsigned int*)ipEntry.ipAddrEnd)))&&
...
int pFwTableContentList(request *wp, int argc, char **argv)
{
	...
	unsigned char *rmtip;
	unsigned char ip_buff[32] = {0}, rmt_ip_buff[64]={0};
#endif
	char enable[64]={0};
	...
	for (i=1; i<=entryNum; i++) {	
		*((char *)&entry) = (char)i;
		...
		if ( !strcmp(rmtip, "0.0.0.0")){			
			rmtip = "<script>dw(portfw_tbl_any)</script>";
...
int ipFilterList(request *wp, int argc, char **argv)
{
	...
	char	*type;
#ifdef IP_RANGE_CMCC_SUPPORT
	int iplen,ipendlen,dstiplen,dstipendlen, mode;
	//char wanBuf[20];
	char ip[16];
	char endip[16] ;
	char dstip[16] ;
	...
	for (i=1; i<=entryNum; i++) {
		*((char *)&entry) = (char)i;
		...
		if ( entry.protoType == PROTO_BOTH )
			...
			type = "ARP";
		else if (entry.protoType == PROTO_ICMP_ARP)
			type = "ICMP+ARP";
#endif
		else
			type = "UDP";
		...
		if(entry.ipVer==IPv4)
			...
			"<td align=center width=\"25%%\" ><font size=\"2\">%s</td>\n"
      			...
      			"<td align=center width=\"10%%\" ><font size=\"2\">%s</td>\n"
      			"<td align=center width=\"10%%\" ><input type=\"checkbox\" name=\"select%d\" value=\"ON\"></td></tr>\n"),
				ip, endip, dstip, dstendip, type, entry.comment, i);
			#else
			nBytesSent += req_format_write(wp, ("<tr class=\"tbl_body\">"
			"<td align=center width=\"30%%\" ><font size=\"2\">%s</td>\n"
      			"<td align=center width=\"25%%\" ><font size=\"2\">%s</td>\n"
			...
			"<td align=center width=\"25%%\" ><font size=\"2\">%s</td>\n"
      			...
      			"<td align=center width=\"10%%\" ><font size=\"2\">%s</td>\n"
      			"<td align=center width=\"10%%\" ><input type=\"checkbox\" name=\"select%d\" value=\"ON\"></td></tr>\n"),
				entry.ip6Addr, endip, dstip, dstendip, type, entry.comment, i);
			#else
			nBytesSent += req_format_write(wp, ("<tr class=\"tbl_body\">"
			"<td align=center width=\"30%%\" ><font size=\"2\">%s</td>\n"
      			"<td align=center width=\"25%%\" ><font size=\"2\">%s</td>\n"
...
int ipQosList(request *wp, int argc, char **argv)
{
	...
	strcpy(_mode1, ("<script>dw(ip_qos_quarant_minband)</script>"));
	strcpy(_mode2, ("<script>dw(ip_qos_restrict_maxband)</script>"));
