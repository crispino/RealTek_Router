...
int  opModeHandler(request *wp, char *tmpBuf)
{
	...
	{
		int rptr_enabled = 1, wlan_disabled = 0;
		int BUF_SIZE = 20;
		...
		apmib_set(MIB_RTL_LINK_MODE, (void *)&rptr_enabled);
#endif		
...
void formReboot(request *wp, char *path, char *query)
{
	...
	submitUrl = req_get_cstream_var(wp, "submit-url", ""); 
	strRequest = req_get_cstream_var(wp, ("reboot"), "");
	if (strRequest[0] && strcmp(strRequest,"Reboot") == 0) {
...
#endif
#ifdef HOME_GATEWAY
		snprintf(tmpBuf,sizeof(tmpBuf), "%s","The Router is booting.<br>Do not turn off or reboot the Device during this time.<br>");
#else
		snprintf(tmpBuf,sizeof(tmpBuf), "%s", "The AP is booting.<br>");
#endif
		apmib_update_web(CURRENT_SETTING);	// update configuration to flash
		apmib_get( MIB_IP_ADDR,  (void *)lan_ip_buf);
...
void formTracert(request *wp, char *path, char *query)
{
	...
	wan_ifac = req_get_cstream_var(wp, ("wanifac"), "");
	target_addr = req_get_cstream_var(wp, ("target_addr"), "");
	if(target_addr == NULL || !target_addr[0]) {	
		printf("[%s:%d] Error, target_addr is null!\n", __FUNCTION__, __LINE__);
		return;
	}


	if(wan_ifac&&wan_ifac[0]&&memcmp(wan_ifac,"default",strlen("default"))) {
		char * strIdx=NULL;
		if((strIdx=strsep(&wan_ifac,"_"))==NULL){
			return 0;
		}
		
		wan_num=strtoul(strIdx,NULL,10);
		wan_num = (wan_num==1)?1:wan_num+6;// wan1->eth1,wan2->eth(8+0),wan3->eth(8+1)
		snprintf(buf, sizeof(buf), "traceroute -i eth%d %s 2>&1", wan_num, target_addr);
	}
	else
		snprintf(buf, sizeof(buf), "traceroute %s 2>&1", target_addr);
	...
	while(!feof(fp)) {
	    if(fgets(buf,sizeof(buf),fp)) {
			req_format_write(wp, ("%s<br>"), buf);			
			//printf("[traceroute]: %s",buf);
	    }
	}
	pclose(fp);
}
...
void formPing(request *wp, char *path, char *query)
{
	...
	wan_ifac = req_get_cstream_var(wp, ("wanifac"), "");
	target_num = req_get_cstream_var(wp, ("target_num"), "");
	
	if(target_num == NULL ) {
		printf("[%s:%d] Error, target_num is null!\n", __FUNCTION__, __LINE__);
	}
	...
	if(wan_ifac&&wan_ifac[0]&&memcmp(wan_ifac,"default",strlen("default"))) {
		...
		wan_num=strtoul(strIdx,NULL,10);
		wan_num = (wan_num==1)?1:wan_num+6;// wan1->eth1,wan2->eth(8+0),wan3->eth(8+1)
		snprintf(buf, sizeof(buf), "ping -I eth%d -c %d -W 3 %s 2>&1", wan_num, num, target_addr);
	}
	else
		snprintf(buf, sizeof(buf), "ping -c %d -W 3 %s 2>&1", num, target_addr);//-W 3:timeout is 3s
...
void formRebootCheck(request *wp, char *path, char *query)
{
...
#ifdef RTK_REINIT_SUPPORT
	{
		...
		)
		{
			...
			snprintf(tmpBuf, sizeof(tmpBuf), "%s","<h4><script>dw(wlsurvey_change_setting_success)</script><br><br><script>dw(wlsurvey_donot_turn_off)</script></h4>");//mzh
...
void formDosCfg(request *wp, char *path, char *query)
{
	...
	if(!strcmp(tmpStr, "ON")) {
		...
		tmpStr = req_get_cstream_var(wp, ("Winnuke"), "");
		if(!strcmp(tmpStr, "ON")) {
			enabled |= 0x80000;
		}
		else{
		...
		tmpStr = req_get_cstream_var(wp, ("ICMP_Redirection"), "");
		if(!strcmp(tmpStr, "ON")) {
			enabled |= 0x100000;
		}
		else{
	...
	sprintf(tmpMsg, "%s","<script>dw(dos_change_setting_success)</script><br><br><script>dw(saveconf_not_turnoff_reboot)</script>");
	apmib_get( MIB_IP_ADDR,  (void *)lan_ip_buf) ;
	sprintf(lan_ip,"%s",inet_ntoa(*((struct in_addr *)lan_ip_buf)) );
	OK_MSG_FW(tmpMsg, submitUrl,APPLY_COUNTDOWN_TIME,lan_ip);
...
#define MAX_USER				LOGIN_URL_MAX_USER
#define ACCESS_TIMEOUT	 			LOGIN_URL_ACCESS_TIMEOUT
...
void delete_user(request *wp)
{
	...
	for (i=0; i<MAX_USER; i++) {
		if(auth_get_clone_mac_by_ip(wp->remote_ip_addr,mac) < 0)
			return -1;
		
		if (users[i].flag == MAGIC_NUMER && !strcmp(mac, users[i].remote_mac)) {
			users[i].flag = 0;
			return;
		}			
	}
...
int add_user(request *wp)
{
	...
	for (i=0; i<MAX_USER; i++) {
		...
		if (users[i].flag == MAGIC_NUMER && strcmp(mac, users[i].remote_mac) &&
			((unsigned long)wp->time_last)-((unsigned long)users[i].last_time) < ACCESS_TIMEOUT )
			continue;
				
		users[i].flag = MAGIC_NUMER;		
		users[i].last_time = wp->time_last;	
		strncpy(users[i].remote_mac,mac,sizeof(users[i].remote_mac));
		return 0;
	}
...
int is_valid_user(request *wp)
{
	...
	for (i=0; i<MAX_USER; i++) {
		...
		if (users[i].flag == MAGIC_NUMER && !strcmp(mac, users[i].remote_mac)) {
			if (((unsigned long)wp->time_last)-((unsigned long)users[i].last_time) > ACCESS_TIMEOUT)
				return -1; // timeout
			return 1;
		}
	}
