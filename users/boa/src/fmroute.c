		...
		*((char *)&entry) = (char)i;
		...
		nBytesSent += req_format_write(wp, "<tr class=\"tbl_body\">"
			"<td align=center width=\"20%%\" ><font size=\"2\">%s</td>\n"
				...
				ip, netmask, gateway,entry.metric,(entry.interface?"WAN":"LAN"), entry.Enable?"<script>dw(route_enable)</script>":"<script>dw(route_disable)</script>",i);
		#else
		nBytesSent += req_format_write(wp, "<tr class=\"tbl_body\">"
...
int kernelRouteList(request *wp, int argc, char **argv)
{
	...
	while (fgets(buff, sizeof(buff), fp) != NULL) {
		if (nl) {
			...
			sdest, sgw, inet_ntoa(mask), metric, (entry.interface?"WAN":"LAN"),(entry_type?"<script>dw(routetbl_static)</script>":"<script>dw(routetbl_dynamic)</script>"));
			}
		}
		nl++;
	}
