...
void set_lan_dhcpd(char *interface, int mode)
{
	...
	if((opMode==BRIDGE_MODE) && (dhcpMode==15))
	{
...
int Domain_query_Process()
{
	...
	if((Operation_Mode==BRIDGE_MODE) && ((WLAN_Mode == 1 && lan_dhcp==15)|| (WLAN_Mode == 0 && lan_dhcp==15))){ //in bridge mode and wlan ap/client mode 
...
void Reset_Domain_Query_Setting(void)
{
	...
	if((opmode==BRIDGE_MODE) &&((wlan_mode_root==1 && lan_dhcp_mode==15) || (wlan_mode_root==0 && lan_dhcp_mode==15)) && lan_domain_name[0]){
