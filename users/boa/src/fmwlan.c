...
// for 11N
	...
	if (strVal[0]) {
		...
		{
			val = 0;	// 40MHz only, disable coexist
		...
		{
			//  20/40M  means is bandwidth is 40M and 20/40 coexist is enabled
			val = 1;
			apmib_set(MIB_WLAN_COEXIST_ENABLED, (void *)&val);
		}
		else {
...
/* SSID Priority */
#ifdef CONFIG_CMCC
	sprintf(varName, "ssidpri%d", wlan_id); 
	strVal = req_get_cstream_var(wp, varName, "");	
	if (strVal[0]) {
		if ( strVal[0] == '0')
			val = 0;
			...
			strcpy(tmpBuf, ("Error! Invalid SSID Priority."));
			goto setErr_wlan;
		}
		
		if ( apmib_set(MIB_WLAN_MANUAL_PRIORITY, (void *)&val) == 0) {
			strcpy(tmpBuf, ("Set MIB_WLAN_MANUAL_PRIORITY failed!"));
			goto setErr_wlan;
		}
	}
#endif
...
#ifdef CONFIG_CMCC
		int oper_mode;		
		apmib_get(MIB_OP_MODE, (void *)&oper_mode);		
#endif
    	sprintf(varName, "repeaterEnabled%d", wlan_id);
    	...
    	if ((strVal==NULL || strVal[0]==0)   // not called from wizard	
    		...
    		if ( !strcmp(strVal, "ON")){
                ...
                {              
					...
					if(repeaterMode==1 && oper_mode==BRIDGE_MODE && (*mode!=CLIENT_MODE))
						val = 1;
					else
#endif
                    	val = 0 ;				
    		...
    		if (*mode == AP_MODE || *mode == AP_WDS_MODE || *mode == AP_MESH_MODE)
    		    apmib_set(id, (void *)&val);
    		if (val == 1) {				
#if 0//def CONFIG_CMCC
				if(oper_mode!=REPEATER_MODE)
#endif
				{
	    			sprintf(varName, "repeaterSSID%d", wlan_id);
	    			strVal = req_get_cstream_var(wp, varName, NULL);
...
#if defined(CONFIG_CMCC)||defined(SYNC_VXD_TO_ROOT)
// not concern with wapi and 1x
int sync_vxd_to_root(unsigned int encrypt)
{
...
void formAndlink(request *wp, char *path, char *query)
{
	char *str_enable_andlink,*str_andlink_mode,*str_local_ip_addr,*submitUrl,*str_andlink_auto_dhcp;
	char *str_andlink_version,*str_andlink_sync,*str_andlink_hb_port,*str_connect_gw_timeout,*str_get_ip_timeout,*str_andlink_recovery;
	int andlink_enable,andlink_mode,sync,andlink_hb_port,connect_gw_timeout,get_ip_timeout,recovery,auto_dhcp;
	struct in_addr local_ip_addr;
	int pid=-1;
	char strPID[10];

	str_enable_andlink = req_get_cstream_var(wp, ("andlink_"), "");
	if (str_enable_andlink[0]) {
		if (!strcmp(str_enable_andlink, ("enable")))
			andlink_enable = 1;
		else
			andlink_enable = 0;
		apmib_set(MIB_RTL_LINK_ENABLE,(void *)&andlink_enable);
	}

	str_andlink_mode = req_get_cstream_var(wp, ("andlink_mode"), "");
	if (str_andlink_mode[0]) {
		if (!strcmp(str_andlink_mode, ("bridge")))
			andlink_mode = 0;
		else
			andlink_mode = 1;
		apmib_set(MIB_RTL_LINK_MODE,(void *)&andlink_mode);
	}

	str_local_ip_addr = req_get_cstream_var(wp, ("local_ip_addr"), "");
	if (str_local_ip_addr[0]) {
		inet_aton(str_local_ip_addr,&local_ip_addr);
		apmib_set(MIB_RTL_LINK_LOCAL_IP_ADDR,(void *)&local_ip_addr);
	}

	str_andlink_version = req_get_cstream_var(wp, ("andlink_version"), "");
	if (str_andlink_version[0]) {
		printf("%s::::%d,andlink_version=%s\n",__FUNCTION__,__LINE__,str_andlink_version);
		apmib_set(MIB_RTL_LINK_ANDLINK_VER,(void *)str_andlink_version);
	}

	str_andlink_sync = req_get_cstream_var(wp, ("andlink_search_sync"), "");
	if (str_andlink_sync[0]) {
		if (!strcmp(str_andlink_sync, ("sync")))
			sync = 0;
		else
			sync = 1;
		apmib_set(MIB_RTL_LINK_SEARCH_ASYNC,(void *)&sync);
	}

	str_andlink_hb_port = req_get_cstream_var(wp, ("andlink_heartbeat_port"), "");
	if (str_andlink_hb_port[0]) {
		apmib_set(MIB_RTL_LINK_HBPORT,(void *)str_andlink_hb_port);
	}
	
	str_connect_gw_timeout = req_get_cstream_var(wp, ("andlink_connect_gw_timeout"), "");
	if (str_connect_gw_timeout[0]) {
		string_to_dec(str_connect_gw_timeout, &connect_gw_timeout);
		apmib_set(MIB_RTL_LINK_CONNECT_GW_TIMEOUT,(void *)&connect_gw_timeout);
	}
	str_get_ip_timeout = req_get_cstream_var(wp, ("andlink_get_ip_timeout"), "");
	if (str_get_ip_timeout[0]) {
		string_to_dec(str_get_ip_timeout, &get_ip_timeout);
		apmib_set(MIB_RTL_LINK_GETIP_TIMEOUT,(void *)&get_ip_timeout);
	}
	str_andlink_recovery = req_get_cstream_var(wp, ("andlink_recovery"), "");
	if (str_andlink_recovery[0]) {
		if (!strcmp(str_andlink_recovery, ("disable")))
			recovery = 0;
		else
			recovery = 1;
		apmib_set(MIB_RTL_LINK_NEED_RECOVERY,(void *)&recovery);
	}

	str_andlink_auto_dhcp = req_get_cstream_var(wp, ("andlink_auto_dhcp"), "");
	if (str_andlink_auto_dhcp[0]) {
		if (!strcmp(str_andlink_auto_dhcp, ("disable")))
			auto_dhcp = 0;
		else
			auto_dhcp = 1;
		apmib_set(MIB_RTL_LINK_AUTO_DHCP,(void *)&auto_dhcp);
	}
...
int wirelessClientList(request *wp, int argc, char **argv)
{
	...
	for (i=1; i<=MAX_STA_NUM; i++) {
		...
		if(pInfo->txOperaRates >= 0xA0) {
			...
			txrate,
			( (pInfo->flag & STA_INFO_FLAG_ASLEEP) ? "<script>dw(wlstatbl_yes)</script>" : "<script>dw(wlstatbl_no)</script>"),
...
#ifdef CONFIG_CMCC
		sprintf(varName, "wl_pri_ssid%d", idx);
		strVal = req_get_cstream_var(wp, varName, "");
		if (strVal[0]) {
			if ( strVal[0] == '0')
				val = 0;
				...
				val = 7;			
			apmib_set(MIB_WLAN_MANUAL_PRIORITY, (void *)&val);
		}
#endif
...
#ifdef SYNC_VXD_TO_ROOT
				sync_vxd = req_get_cstream_var(wp, ("sync_vxd"), "");
				if(sync_vxd[0]){
					is_sync_vxd = 1;
				}
#endif
...
//printf("\r\n ++++++++++++wlanifp=[%s],__[%s-%u]\r\n",wlanifp,__FILE__,__LINE__);
        ...
        {                
            ...
            apmib_set(MIB_WLAN_PREFER_BSSID, (void *)pBss->bdBssId);
...
#ifdef CONFIG_CMCC	
		//it should Customize SSID setting after WPS reset to unconfigured mode.
		intVal = 0;
		apmib_set(MIB_SSID_VER, (void *)&intVal);
#endif
...
#ifndef NO_ACTION
				sprintf(tmpbuf, "echo 1 > /var/wps_start_interface%d", wlan_idx);
				system(tmpbuf);
