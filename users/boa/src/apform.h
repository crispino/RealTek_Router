...
#define MAIN_PAGE "/main.html"
#define HOMGPAGE LOGIN_PAGE
#else
#define HOMGPAGE "home.htm"
#endif
...
#define ERR_MSG(msg) { \
	...
	req_format_write(wp, "<form name=\"setok\"><input type=\"button\" name=\"ok_msg\"onclick=\"history.go (-1)\" value=\"&nbsp;&nbsp;OK&nbsp;&nbsp\" name=\"OK\"><script>document.setok.ok_msg.value = wlsurvey_connect_ok;</script></form></blockquote></body></html>"); \
}
...
#define OK_MSG_FW(msg, url, c, ip) { \
	...
	req_format_write(wp, "<P align=left><h4><script>dw(wlsurvey_please_wait)</script><B><SPAN id=show_sec></SPAN></B>&nbsp;<script>dw(wlsurvey_seconds)</script> ...</h4></P>");\
...
#define OK_MSG_FW(msg, url, c, ip) { \
	...
	req_format_write(wp, "var count = %d;function get_by_id(id){with(document){return getElementById(id);}}\n", c);\
   	req_format_write(wp, "function do_count_down(){get_by_id(\"show_sec\").innerHTML = count\n");\
	req_format_write(wp, "if(count == 0) {parent.location.href='http://%s/top_cmcc.asp?t='+new Date().getTime(); return false;}\n", ip);\
	req_format_write(wp, "if (count > 0) {count--;setTimeout('do_count_down()',1000);}}");\
	req_format_write(wp, "//-->\n");\
	req_format_write(wp,"</script></head>");\
	req_format_write(wp, "<body onload=\"do_count_down();\"><blockquote><h4>%s</h4>\n", msg);\
...
#endif
#ifdef CONFIG_APP_BOA_HE_UI
extern int getHeInfo(request *wp, int argc, char **argv);
extern int dhcpWiredClientList(request *wp, int argc, char **argv);
extern int dhcpMacCloneClientList(request *wp, int argc, char **argv);
extern int getdhcpWiredClientnum(request *wp, int argc, char **argv);
extern int getdhcpWiredClientIPMAC(request *wp, int argc, char **argv);
extern int dhcpWirelessClientList(request *wp, int argc, char **argv);
extern void QuickSetUp(request *wp, char *path, char *query);
extern void netState(request *wp, char *path, char *query);
extern void netDiagnosis(request *wp, char *path, char *query);
extern void doInitInternet(request *wp, char *path, char *query);
extern void heUiReboot(request *wp, char *path, char *query);
extern void heUIUpload(request *wp, char *path, char *query);
extern void dhcpbanding(request *wp, char *path, char *query);
extern void MACCloneCfg(request *wp, char *path, char *query);
extern void setWan(request *wp, char *path, char *query);
extern void entconfig(request *wp, char *path, char *query);
extern void LoadDefaultSettings(request *wp, char *path, char *query);
extern void setSysAdm(request *wp, char *path, char *query);
extern void he_form_syslog(request *wp, char *path, char *query);
extern void he_setSysTime(request *wp, char *path, char *query);
extern void formwanconfig(request *wp, char *path, char *query);
extern void he_form_wirelessSet(request *wp, char *path, char *query);
extern void he_form_wirelessQuick(request *wp, char *path, char *query);
extern void he_form_wireless_channel(request *wp, char *path, char *query);
extern void he_form_wireless_guest(request *wp, char *path, char *query);
extern void he_form_wps_setting(request *wp, char *path, char *query);
extern void he_form_wps_config(request *wp, char *path, char *query);
extern void disconnectbymacfilter(request *wp, char *path, char *query);
extern int draw_wlSiteSurveyTbl(request *wp, int argc, char **argv);
extern void heUiOpMode(request *wp, char *path, char *query);
extern void heUiAutoUpload(request *wp, char *path, char *query);
extern void he_wirelessGetMAClist(request *wp, char *path, char *query);
extern void he_APSecurity(request *wp, char *path, char *query);
extern void he_APBatchDeleteAccessPolicyList(request *wp, char *path, char *query);
extern void heUIIpfilter(request *wp, char *path, char *query);
extern void heUIURLfilter(request *wp, char *path, char *query);
extern void heUIMACfilter(request *wp, char *path, char *query);
extern void heUIformALGOnOff(request *wp, char *path, char *query);
extern void heUITelnetd(request *wp, char *path, char *query);


#endif
#ifdef CONFIG_APP_CMCC_GW_UI
extern void formFastConf(request *wp, char *path, char *query);
...
extern void formAndlink(request *wp, char *path, char *query);
...
extern void frmWanIpv6Setup(request *wp, char * path, char * query);
extern void frmLanIpv6Setup(request *wp, char * path, char * query);
extern void frmIpv6DsliteSetup(request *wp, char * path, char * query);
