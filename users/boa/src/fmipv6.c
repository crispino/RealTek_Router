...
/**
from: fe80000000000000b2d59dfffebacd59
to: fe80::b2d5:9dff:feba:cd59
**/
void getFormatIPv6Addr(const char* src, char* dst)
{
	int i = 0;
	int match = 0;
	int flag = 0;
	...
	if (strlen(src) < 32) return;
	
	while(i < 8) {
		//1. continue 0000 flag check
		if (flag) {
			if (!strncmp(src + i*4, "0000", 4)) {
				i++;
				continue;
			}
		}
		...
		//2. :: matched
		if (!match && !strncmp(src + i*4, "0000", 4) && !strncmp(src + (i+1)*4, "0000", 4)) {
			if (i == 0) {
				strncpy(p, "::", 2);
				p += 2;
			} else {
		...
		strncpy(tmp, src + 4*i, 4);
		tp = tmp;
		while(*tp == '0') tp++;
		
		if (strlen(tp) > 0) {
			strncpy(p, tp, strlen(tp));
			p += strlen(tp);
		} else {
			strncpy(p, "0", 1);
			p++;
			...
			while((ret=fscanf(fp,"%s %x %x %x %x %s",src,&if_index,
				...
				if(strcmp(ifname,devname)==0){									
					...
					if(addr6.s6_addr16[0]!=ntohs(0xFE80)){
						memset(tmpstr, 0, sizeof(tmpstr));
						getFormatIPv6Addr(src, tmpstr);
						req_format_write(wp,"%s/%d",tmpstr,prefix_len);
						break;
					}
				}
			}
	...
	else if(!strcmp(name,"wan-dns-ipv6"))
	{
		...
		else if(val==1) ////Attain DNS Automatically 
		{
			...
			if(!apmib_get(MIB_IPV6_DHCP_MODE,&val)){
				...
				if (val==1) {// stateful mode, get name server from dns6.config
...
void dhcpv6Handler(request *wp)
{
	char* strval = NULL;
	int val = 0;

	//1. set origin Type and link Type
	val = 0;
	apmib_set(MIB_IPV6_ORIGIN_TYPE, &val);
...
void staticIpv6Handler(request *wp)
{
	addr6CfgParam_t addr6_wan;
	addr6CfgParam_t addr6_gw;
	addr6CfgParam_t addr6_dns;
	...
	//3. set wan ip address
	strval = req_get_cstream_var(wp, "wan_ip_0", "");
	printf("%s:%d, wan_ip_0 :%s\n", __FUNCTION__, __LINE__, strval);
	if(strval!=NULL)
		addr6_wan.addrIPv6[0]=strtol(strval,NULL,16);
	strval=req_get_cstream_var(wp,"wan_ip_1","");	
	if(strval!=NULL)
	...
	//4. set wan gw ip address
	strval=req_get_cstream_var(wp,"wan_gw_0","");
	printf("%s:%d, wan_gw_0 :%s\n", __FUNCTION__, __LINE__, strval);
	if(strval!=NULL)
		addr6_gw.addrIPv6[0]=strtol(strval,NULL,16);	
	strval=req_get_cstream_var(wp,"wan_gw_1","");
	if(strval!=NULL)
...
void dsliteHandler(request *wp)
{
	...
	//3. set ds-lite mode
	strval = req_get_cstream_var(wp, "dsliteMode", "");
	printf("%s:%d, dsliteMode :%s\n", __FUNCTION__, __LINE__, strval);
	if(strval[0]) {
		if(strcmp(strval,"dsliteManual") == 0) {
			val = 1;
		} else {
			val = 0;			
		}
		apmib_set(MIB_DSLITE_MODE, &val);
	}
#endif
	...
	char *strAFTR;
	DHCP_T dhcp = AFTR;
	char* strMode = req_get_cstream_var(wp, ("dsliteMode"), "");
...
void frmIpv6DsliteSetup(request *wp, char *path, char *query)
{
	int needReinit = 1;
	char *submitUrl = NULL;
	char* strval = NULL;
	...
	//1. set dslite_enable
	strval = req_get_cstream_var(wp,"dslite_enable","");
	printf("%s:%d, dslite_enable :%s\n", __FUNCTION__, __LINE__, strval);
	val = atoi(strval);
	if (val == 0) {
		needReinit = 1;
		dhcp = DHCP_CLIENT;
...
EXIT_DSLITESETUP:
	...
	if (submitUrl[0]) send_redirect_perm(wp, submitUrl);
...
void frmWanIpv6Setup(request *wp, char *path, char *query)
{
	int needReinit = 1;
	char *submitUrl = NULL;
	char* strval = NULL;
	uint32 val, originType;

	printf("%s:%d, enter\n", __FUNCTION__, __LINE__);

	//1. set wan_enable
	strval = req_get_cstream_var(wp,"wan_enable","");
	printf("%s:%d, wan_enable :%s\n", __FUNCTION__, __LINE__, strval);
	val = atoi(strval);
	apmib_set(MIB_IPV6_WAN_ENABLE, &val);
	if (val == 0) {
		needReinit = 0;
...
void frmLanIpv6Setup(request * wp, char * path, char * query)
{
	int needReinit = 1;
	dhcp6sCfgParam_t dhcp6sCfgParam;
	addr6CfgParam_t	addr6;
	...
	if(enable) {
		...
		//default dns address local ip address
		memset(tmpBuf, 0, sizeof(tmpBuf));
		sprintf(tmpBuf,"%04x:%04x:%04x:%04x:%04x:%04x:%04x:%04x",
						addr6.addrIPv6[0],addr6.addrIPv6[1],addr6.addrIPv6[2],
						addr6.addrIPv6[3],addr6.addrIPv6[4],addr6.addrIPv6[5],
						addr6.addrIPv6[6],addr6.addrIPv6[7]);
		strcpy(dhcp6sCfgParam.DNSaddr6, tmpBuf);
