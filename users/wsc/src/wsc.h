...
#define MESH_SUPPORT
#ifdef MESH_SUPPORT
#define MESH_SUPPORT_TEST
#endif

//#define STA_CONTROL
#ifdef STA_CONTROL
#define STA_CONTROL_TEST
#endif
...
#define MAX_MESHPROFILE_LEN			200
...
#define PBC_WALK_TIME				(pCtx->reg_timeout) // in sec

//#define PIN_WALK_TIME				120 // in sec
#define PIN_WALK_TIME				(pCtx->reg_timeout)
...
#define SETSELREG_WALK_TIME		(pCtx->reg_timeout)
...
#define TAG_RTK_PROPRIETARY				0x106D	//propietary TAG
#define TAG_RTK_PROPRIETARY_1			0x106E	//propietary TAG
#define TAG_RTK_PROPRIETARY_2			0x106F	//propietary TAG
#define TAG_RTK_PROPRIETARY_3			0x1070	//propietary TAG
#define TAG_MESH_RESERVED				0x1071  //propietary TAG for customer to use
...
#define PARAM_MESH_TEMP_FILE		("/tmp/flash_param_mesh")
#define WSCD_MESH_CONFIG_STATUS		("/tmp/wscd_mesh_status")
#endif
//#define FLASH_PARAM_W_INTF			1
#ifdef	DET_WPS_SPEC
...
#define LED_WSC_ERROR				TURNKEY_LED_WSC_NOP
#define LED_WSC_SUCCESS			TURNKEY_LED_WSC_NOP
#if defined(MESH_SUPPORT)
	#define LED_MESH_WSC_ENROLLEE_START		TURNKEY_LED_MESH_WSC_ENROLLEE_START
	#define LED_MESH_WSC_REGISTRAR_START	TURNKEY_LED_MESH_WSC_REGISTRAR_START
#endif
#endif
...
#endif
#if defined(MESH_SUPPORT)
	#define TURNKEY_LED_MESH_WSC_ENROLLEE_START	-8
	#define TURNKEY_LED_MESH_WSC_REGISTRAR_START	-9
#endif

#if defined(MESH_SUPPORT)
enum { REG_AP=1, REG_MESH, ENROLLEE_MESH };
enum { MIB_TYPE_CHAR, MIB_TYPE_INT, MIB_TYPE_BYTE6, MIB_TYPE_STR };
#endif
...
typedef enum { SCAN_2_CONNECT=1, SCAN } SCAN_MODE_T;
...
#if defined(MESH_SUPPORT)
struct _mesh_profile {
	unsigned char id[WSC_MAX_SSID_LEN+1];
	//int enabled, channel, id_len, network_key_len; 
	//unsigned short enc, cipher;
	unsigned int enabled, channel, id_len, network_key_len, enc, cipher;	
	unsigned char network_key[MAX_NETWORK_KEY_LEN+1];
	unsigned char reserved[MAX_MESH_RESERVED_LEN];
	int reserved_len;
};
...
#if defined(STA_CONTROL)
struct _bandsteering_profile {
	int enabled, prefer;
};
#endif
#endif
...
typedef struct context {
...
#if defined(STA_CONTROL)
	struct _bandsteering_profile bandsteering_profile[2];
#endif
#ifdef FOR_DUAL_BAND
	int socket2;
	...
	unsigned char both_band_credential_mesh;
#endif
	...
	char wlan_interface_name_bak[40];
	...
	int MeshMode2;
	int new_flash_write;
	/*  for MeshMode==ENROLLEE_MESH */
	int mesh_en_timeout;
	int mesh_reg_timeout;
	int curr_mesh_poll_role;
	int mesh_wps_initial_proc;
	int mesh_wps_timeout;
	/*  for MeshMode==ENROLLEE_MESH */
	int dfs_band;  // -1:no dfs band, 0:wlan0 is dfs band, 1:wlan1 is dfs band
	int disable_dfs_bak;
	char wlan0_mac[MACADDRLEN];
	char wlan1_mac[MACADDRLEN];	
#endif
	int upnp;
	int role;
	int original_role;
	...
	int network_key_len;
#if defined(MESH_SUPPORT)	
	struct _mesh_profile mesh_profile[2];
#endif
#if defined(MESH_SUPPORT) || defined(STA_CONTROL)
	struct _rtk_tlv rtk_tlv[2];
#endif
	// FOR_DUAL_BAND
#ifdef FOR_DUAL_BAND
...
#if defined(MESH_SUPPORT)
	int MESH_WPS_REGISTRAR_START_LED_GPIO_number;
	int MESH_WPS_ENROLLEE_START_LED_GPIO_number;
	int mesh_er_button_hold_time;
	int mesh_ee_button_hold_time;
#endif
#ifdef	AUTO_LOCK_DOWN
...
#ifdef WSC_CLIENT_MODE
#if defined(MESH_SUPPORT)
extern int issue_scan_req(CTX_Tp pCtx, int method, int mode);
#else
...
#if defined(MESH_SUPPORT)
extern int pbs_as_mesh_role(CTX_Tp pCtx,int mesh_role);
extern int init_config(CTX_Tp pCtx,int def);
extern int rollback_role(CTX_Tp pCtx);
extern void remove_active_pbc_sta_all(CTX_Tp pCtx);
extern void remove_active_pbc_sta_by_uuid(CTX_Tp pCtx, unsigned char* staUuid , unsigned char mode);
extern int wps_as_mesh_role(CTX_Tp pCtx, int mesh_role);
extern void process_event(CTX_Tp pCtx, int evt);
extern int generate_rtk_tlv(CTX_Tp pCtx, int index);
extern int is_mesh_connect(CTX_Tp pCtx);
extern int transform(char *source, char *target,int intface);
extern int mesh_get_reg_en_timeout(CTX_Tp pCtx);
extern int write_param_to_flash_mesh(CTX_Tp pCtx, int is_local, int cred_idx, int intface);
extern int is_mesh_sta(CTX_Tp pCtx, unsigned char *mac);
extern int enable_reg_ap_mode(CTX_Tp pCtx);
extern void add_mesh_vendor_ie(CTX_Tp pCtx, unsigned char select);
extern void clear_mesh_vendor_ie(CTX_Tp pCtx);
extern int get_wlan_dfs(CTX_Tp pCtx);
extern int proc_DFS(CTX_Tp pCtx, int value);
extern int is_valid_macaddr_uuid(CTX_Tp pCtx, unsigned char *addr, unsigned char *uuid);
extern int iwpriv_set_mib(CTX_Tp pCtx, char *mibName, int value);
#endif
