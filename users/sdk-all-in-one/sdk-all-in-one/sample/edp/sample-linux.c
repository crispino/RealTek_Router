...
int main( int argc, char *argv[] )
{
    ...
    err = ont_device_create(1832, ONTDEV_EDP, "auto_register", &dev);
    if (ONT_ERR_OK != err) {
    ...
    err = ont_device_connect_without_autoReg(dev,
	    ...
	    ONT_SERVER_PORT,
	    "omq_test",
	    "{\"SN\":\"12211706888888te\",\"MAC\":\"C8:3A:35:48:B2:35\"}",
	    30);
    if (ONT_ERR_OK != err)
    {
	...
	if (next_dp_time <= time(NULL))
	{
	    ...
	    err = ont_edp_send_transdata(dev,"myservice",trans_data,strlen(trans_data));
	    if (ONT_ERR_OK != err)
	    {
