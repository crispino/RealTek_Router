...
static device_onvif_discovery_list_t _gOnvifDeviceDiscovery = { 0 };
        ...
        while (retval == SOAP_OK)
        {
            ...
            if (retval == SOAP_OK)
            {
                ...
                else //成功接收某一个设备的消息
                {
                    ...
                    {
                        device_onvif_discovery_info_t* dodiPtr = getDeviceOnvifDiscoveryPtr(&_gOnvifDeviceDiscovery, &HasDev);
...
int ont_onvifdevice_checkdevice_online(const char*url)
{
    char * p = NULL;
    int i = 0;
    char tempurl[256];
    ...
    for (i = 0; i < _gOnvifDeviceDiscovery.number && _gOnvifDeviceDiscovery.list[i].srvAddr[0] != '\0'; i++) {
        char *srvAddr = _gOnvifDeviceDiscovery.list[i].srvAddr;
